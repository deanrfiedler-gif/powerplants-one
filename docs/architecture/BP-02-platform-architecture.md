...
**Edition:** r11 · **Date:** 6 September 2026 · **Scope:** PP-01 synthetic planned service and BP-03 I1–I2 opportunity prototype.
...
P08 implements assigned job-pack/history reading, pack acknowledgement intent, local attendance intent and capture of time, parts, observations, readings, checklist results and photos. Report-response evidence remains P09. Issuing packs/reports, confirming bookings, approving scope, Finance processing and source refresh require the server. Offline customer response binds an already-issued/cached report revision; local draft work can instead be acknowledged as a draft evidence snapshot with an explicit label and later review.
...
[ADR-0013](../decisions/ADR-0013-p08-offline-recovery.md) and [handover](../delivery/p08-handover.md) implement a dedicated `/offline/` shell, content-hashed worker cache, versioned native IndexedDB originals/Blobs/status/leases and bounded explicit API-C15 replay. Current domain authorisation and existing operation/workspace transaction locks remain authoritative. An AsyncLocalStorage hook atomically retains the original envelope with domain/audit/receipt/outbox facts, preserving existing hashes. Original causal references resolve without rewriting queued authority. No new runtime package is added.

The [exact physical dictionary](../contracts/service-data-dictionary.md#p08-physical-implementation-amendment) defines two downloaded jobs, a 24-hour synthetic verification horizon, a 30-second reclaimable sender lease, 20-operation/6 MiB batches and the seven-day original-actor/job-bound restricted recovery capability. These parameters are prototype choices. Unsent evidence survives sign-out/identity locks; normal record/file/receipt access still requires current capability. Storage failure never becomes a memory-backed success claim. Real mobile hardware, operational encryption, retention and remote identity remain unverified.

P09 review/report/customer response and P10 Finance remain absent. P08 completion remains a technician draft. The maintained [P09 starter](../delivery/p09-starter-prompt.md) requires verified P08 publication first.
