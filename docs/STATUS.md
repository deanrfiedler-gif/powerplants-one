...
P11 is now invoked under [issue #54](https://github.com/deanrfiedler-gif/powerplants-one/issues/54), from verified main `744b6e6b89e69abdd0fbd4bfdd6ef5ae51e154f3`. P10's authoritative publication is complete. The [P11 handover](delivery/p11-handover.md) records the bounded shared access/validation/recovery checkpoint and incomplete verification. ADR-0018 authorises separately recorded whole-minute Travel as fully NonBillable with no target/posting; Labour F-06 remains unchanged. Eleven focused database cases now pass in original TAP, including original v1 upgrades, actual v2 output/font/byte/offline-size checks and exact review isolation. Both selected UI journeys passed through read-only second-crew capture history on `6b04f30`. An added report-detail traversal on `b5270c8` then found internal review notes visible to that crew viewer; the server projection correction and two additional DB cases pass on `1d47e3`; internal render-attempt scoping is also under verification. The 12 identity/state cases passed on preceding sources; the same checkpoint has 11 passed and one connection-reset failure during a real access-denial read. No permission result was returned by that failed request. All 320 load reads now pass on `b5270c8`, with all sixteen candidate latency groups still missed. The retained full job reached its 90-minute limit; focused checks are now separated into disposable jobs so every retained gate can finish. The exact-presentation HTTP component passes raw TAP on `2ca86b3`. A pipeline shell defect that could mask producer failure is corrected with explicit Bash/pipefail. Full regression and final-source browser verification remain incomplete. PT-27/29 and full integration/output evidence remain open; PT-30 requires P12. [P12 starter](delivery/p12-starter-prompt.md) is preparation only. The external P11 record, not this status note, will establish final publication.
