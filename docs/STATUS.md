...
**Updated:** 12 September 2026 · **Owner:** Dean Fiedler · **Repository:** `deanrfiedler-gif/powerplants-one`, **public** visibility, default branch `main` · **Baseline commit:** `386d9a7` (merge of PR #127) · **Naming:** [PPO-STD-001](standards/naming-conventions.md) / [ADR-0005](decisions/ADR-0005-project-naming-adoption.md)
...
| Item | State | Evidence |
|---|---|---|
| `main` head | `386d9a7` (merge of PR #127, 12 September 2026 11:40 AEST). Its first parent is `ce3a747`, **pushed directly to `main` without a pull request** and carrying unresolved merge conflict markers in this file; repaired by the commit recording this row. | [Commits](https://github.com/deanrfiedler-gif/powerplants-one/commits/main/) · [`ce3a747`](https://github.com/deanrfiedler-gif/powerplants-one/commit/ce3a747b1c9035eb5d105ab249d9e8625aa52ca9) |
...
| Documentation checks | `check_foundation`, `check_prototype`, `check_naming` pass on the repair head (78 parents, 29 decisions, 30 PT procedures, 12 packages, 128 document records, 1,728 local links). **Two Windows portability defects found and not yet fixed:** `check_foundation.py` has seven `read_text()` calls with no `encoding=`, so it aborts under cp1252 unless `PYTHONUTF8=1` is set; and `.gitattributes` pins `*.md`/`*.csv`/`*.json` to LF but not `*.html`, so the three issued HTML references fail the byte check on any clone with `core.autocrlf=true`. | Local run, 12 September 2026 |
...
| Branch protection | **Not enabled — now the highest-priority owner action.** Four merges in three days went past a red or unfinished check (#110, #80, #111/#97, and #108 three minutes after its last commit with its own workflow red); #108 left `main` red. On 12 September `ce3a747` was pushed **straight to `main` with no pull request at all**, putting a file containing merge conflict markers on the default branch; #127 then merged on top of it and carried it forward. Require at least Documentation assurance and Application assurance on `main`; add the compiled suite when `main` is green again. | Repository settings (owner) |
...
| Design adoption (docs) | #122 five-stage CRM board review fixture (draft); PPO-010-CONTAINER-DEC — five estimating propositions (rule provenance, price binding, invariant placement, unknown amounts, money representation), merged to `main` by `ce3a747` without review | Refresh and merge #122 on green once `main` is green. Decide each estimating proposition on its merits; adopting any would widen the increment authorised under #79 and require the contract or policy changes the record lists. Nothing is superseded |
| Merged 11 September | #111, #97, #112, #114, #115, #104, #78, #60, #116, #80, #81, #68, #65, #77, #118, #119, #79, #123, #108, #124 | — |
| Merged 12 September | #126 STATUS snapshot; #127 Engineering create-request receipt, the fix for #125 | Confirm the Engineering job, the compiled suite and the full proof on `386d9a7` before closing #125 |
| Closed as superseded | #58, #82, #63, #67 and the automated CI-fix set #71 #74 #87 #93–#96 #98–#102 | — |
| Issues | **#125 `main` red after #108** — fix merged as #127; the Engineering job, compiled suite and full proof on `386d9a7` still to be confirmed green before closing; #113 CI stall class — acceptance counts met, closing deferred until `main` is green; #85 closed against #86; #54 P11 authoritative record; PPO-002/009/010/011/012/013/015/016 remain discovery records | — |
