...
**8 September direction:** Dean selected private hosted access so he can use PPO from his smartphone away from his computer and colleagues can use a link independently. Hosted evaluation is the target; local rehearsal supports implementation and is not a required user-facing deliverable. The Azure subscription/identity owner, deployed runtime and tester grants remain unresolved. No hosted URL or active access is claimed.

Dean authorised a bounded package defining demonstrations, example records, tester access, costs, deployment and reset. The [package](delivery/private-prototype-demo.md), [scope decision](decisions/private-prototype-demo.md) and [itemised retail-rate model](delivery/private-prototype-demo-costs.json) are prepared against main `923bd9b90412c80782331b7085a6880750f12640`. The proposed core is customer context, owned opportunity follow-up and manual E1 draft estimating for five invited testers/three concurrent users. Local presentation uses the supported loopback mode; hosted evaluation requires separate real identity, remote configuration, protected document storage and epoch-aware preparation/reset implementation.

The planning estimate is A$160.12/month excluding GST with a 25% contingency and assumed A$1.50/US$1 conversion; A$200 is a proposed review budget, not purchase authority or a hard cap. Standard seed contains no opportunities/estimates; the package supplies the explicit preparation recipe. Documentation publication/check evidence belongs to its PR. No application/schema/seed/permission change, hosting, invitation, reset execution, live integration or owner acceptance is delivered. Current P11/P12 prerequisites and parallel work remain unchanged.
