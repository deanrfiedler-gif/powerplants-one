...
## Self-contained VS Code implementation prompt · CS-05

**Revision:** r02  
**Date:** 21 September 2026  
**Product owner:** Dean Fiedler  
**Purpose:** Attach this Markdown file to the VS Code coding chat to implement the native module.  
...
Implement **CS-05 — Facilities and growing areas: registers, detail and forms** in the existing Powerplants One application. Act as a careful senior product engineer, preserving the established domain contracts and native design system. Build functioning, persisted native application behaviour; do not stop at another plan, standalone HTML demonstration or mock screenshot.

This is **r02** and replaces the earlier r01 prompt. Read the complete contract, including §18's exact payload, preview, recovery and audit resolutions. Do not carry forward r01's mandatory FAC-number backfill; the current protected identity registry is deliberately preserved.

The full delivery contract is embedded below. This prompt is self-contained: its companion build plan is useful for review but is not required as a second attachment. Do not shorten it to a generic CRUD task. Its length is intentional and is not constrained by the repository's separate short project-instructions document limit.

### Start with the actual checkout

1. Inspect `git status`, current branch/HEAD, AGENTS.md, README.md and docs/STATUS.md. Preserve unrelated user changes. Do not reset to this prompt's historical checkpoint.
2. Read the source paths identified in the contract, especially approved FAC-D01–03, the CS-05 coverage audit, r03 workspace/map handovers, current Facility commands/reads, shared authority/receipts and E2 discovery context.
3. Inspect current route/component ownership and the resulting database schema plus migration registry/reservations. Determine which reference, service-link and field contracts now exist. Reuse newer compatible implementation; do not duplicate it.
4. Establish a focused CS-05 branch using the existing repository workflow. Record substantive drift, bounded design refinements and the intended migrations/command ownership in the working handover.
5. Implement the work packages in order, checking each functioning slice. Continue through forms, history, integration, recovery and verification. Ask only about a material unresolved business choice that cannot safely be settled from the contract; complete the unblocked work first.

### Delivery rules

- Keep the canonical Facility UUID/site model and Asset identities. Use the approved taxonomy; do not infer a type from a name.
- Preserve current PPO header/logo/navigation and r22 visual conventions. Native pages use the shared shell; they do not reproduce the standalone HTML's scaffolding.
- Use strict server validation, current scope, expected versions, exact review acknowledgements and native original-operation receipts. Browser storage is not authoritative persistence.
- Retain legacy list/detail/create compatibility and prior IDs, sources, grants, receipts and issued snapshots. New Facility data never automatically reprices an estimate or grants permission to enter/work.
- Reuse current dependencies and existing transaction, receipt, source, UI and permission patterns. Do not add a framework or provider integration to solve this bounded module.
- Keep issued reference HTML/evidence unchanged. Update working decisions/data dictionary/status/handover with the actual delivered state and preserved parent requirement IDs.
- Do not send external messages, merge, deploy, connect production providers or mutate operational records merely because this implementation prompt is attached.
- If tests cannot execute, state the exact blocked command/environment and the unverified acceptance cases. Never claim a design inspection or test definition is execution evidence.

### Suggested first coding-chat message

> Implement the attached CS-05 prompt in the current Powerplants One repository. Reconcile the live checkout first, then carry the required native journeys through persistence, permissions, UI, verification and a reviewable handover.
...
Lead with the working user outcome, then report:

1. Implemented routes and journeys, with exact files/branch/commit.
2. Schema/identity/API changes and source-contract departures, if any.
3. Permission, concurrency, receipt and migration evidence, including saved-output preservation.
4. Executed acceptance cases, actual commands/results and desktop/phone captures reviewed.
5. Any failed or unrun checks and the practical limitation they leave.
6. Reviewable handover location and any remaining owner/device acceptance.

Keep implemented, tested, owner-accepted and deployed as separate states. Do not use “100% complete” or “production ready” without the evidence and authority needed for that claim.

**Contract SHA-256:** `ec4be2a880eb33135209001a1bd96f2f00fd656e654f12b873cefb9cf3b83b1f`  
**Companion build plan:** `PPO-CS-05-Facilities-and-Growing-Areas-Native-Build-Plan-r02.md`  
The contract above is complete; opening the companion is optional.
