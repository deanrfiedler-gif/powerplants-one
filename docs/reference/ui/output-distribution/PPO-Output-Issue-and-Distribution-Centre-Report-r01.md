...
The run used Chromium **141.0.7390.37** through the repository-pinned Playwright 1.63.0 on Node **22.22.2**, launched with the documented `PPO_CHROME_PATH` override. The repository pins Node 24.21.0 and the Chrome channel, which the focused workflow installs; that pinned combination has not been executed for this package, and the executed runtime is recorded verbatim in `results.json`.
