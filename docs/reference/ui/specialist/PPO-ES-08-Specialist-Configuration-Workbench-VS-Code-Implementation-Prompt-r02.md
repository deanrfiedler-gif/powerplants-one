---
document_id: PPO-ES08-VSCODE-IMPLEMENTATION-PROMPT
revision: r02
...
status: Attachment-ready implementation instructions; implementation and acceptance pending
repository: https://github.com/deanrfiedler-gif/powerplants-one
inspected_main: 8ed8b0c5dadad9a48d972a9bff39b6953ad4e55b
companion_plan: PPO-ES-08-Specialist-Configuration-Workbench-Native-Build-Plan-r02.md
---
...
Attach this complete Markdown document to the Visual Studio Code Codex chat with the Powerplants One repository open. It is deliberately self-contained and has no 8,000-character limit. The source HTML, model, catalogue, report and changelog are already referenced in the repository. Attach the current r03 HTML and r22 theme board when available for direct comparison; the companion native build plan supplies the same governed contract and source rationale.

Send this message with the attachment:

> Implement ES-08 using the attached Specialist Configuration Workbench instructions. Read the full document and current repository guidance, then complete the native Screen Systems review and synthetic estimate-receiving scope through testing and handover. Reuse the current Powerplants One shell and estimating contracts. Preserve existing work and report source limitations honestly.

## Your implementation task

Act as the senior product engineer responsible for a professional, traceable estimating module. **Implement the native module**, including persistence, all six views, calculation evidence, safe rerun and working synthetic receiving. A plan-only answer, screenshot, standalone HTML, iframe, localStorage calculator, disabled integration scaffold or Configure-only cosmetic pass is not completion.

Read this document fully before editing. Inspect the actual branch, dirty worktree, current instructions, recently merged/concurrent work and test environment. Do not overwrite unrelated edits, reset newer work or assume the inspected commit is current. Use a suitable isolated branch/worktree under the repository's established workflow. Reconcile this brief with deployed ES-02/E1 contracts before creating schema.

Current user instructions take precedence. The companion native build plan governs detailed intent if supplied; this prompt reproduces its core contracts and acceptance checks so it can be attached alone. Existing repository authority/history contracts remain binding. Record material conflicts and resolve the affected implementation explicitly; do not reinterpret a historical approval note as a requirement to ask again before already-authorized development.

Complete S01–S06 in bounded, reviewable increments. First record the route/schema/authority/calculation/receiving decisions, then implement and verify them. Continue through authorized independent work when an external engineering decision is unavailable; keep the affected operational path visibly ineligible without fabricating formulas, approvals or production integrations. Do not merge or deploy as part of this instruction.

The following numbered sections are the implementation contract. Proposed entities/commands describe work to create, not existing capabilities. Acceptance rows are required future evidence, not passes already achieved.
...
Report the native paths and behavior now available, important architecture/migration decisions, actual tests and visual evidence, and any blockers with their precise scope. Link the reviewable branch/PR according to the repository workflow. Include a short deterministic walkthrough covering configure → review → save run → receive → manual edit → rerun/reconcile → lost-response recovery. Identify the exact commit tested.

State separately what is implemented, what was verified, what awaits owner acceptance and what remains ineligible for operational engineering/pricing use. Do not claim all branches validated, approved motor sizing, live catalogue/ERP integration, production readiness or executed tests without evidence. Finish the independent native review and synthetic receiving work before handing back; do not end after proposing the next steps.
