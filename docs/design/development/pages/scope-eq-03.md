# Configuration succession - native design contract

<!-- versioning: git; committed history is authoritative -->

Stable entry: `scope:EQ-03`. Owner: Dean Fiedler. Review: **Draft; owner visual acceptance pending**.
Starting implementation baseline: `ca006fb1fc6b2e2bd2a9bdb509caa276d1052ffc`. Route: `/equipment/[id]?view=configuration`.

## Purpose and page type

Read retained configuration revisions and their effective times. Describe a source-backed successor, review its impact, then submit a separate review decision.

Selected r20 family: **Document & evidence workspace; supporting Review / comparison**. The issued [Equipment r02 HTML](../../../reference/ui/equipment/PPO-Equipment-and-Installed-Base-Workspace-r02.html) remains the semantic/reference source. Its bytes are unchanged. No accepted native image is available; missing desktop/mobile approval images are explicit.
...
1. Enter the exact configuration description, source revision, actual effective time, reason and consequences.
2. Preview the current Asset/configuration/Service/relationship basis; record the proposal.
3. Review the immutable proposal and apply or reject it. A changed source requires a fresh proposal.
...
Service scopes retain exact AssetConfiguration IDs; Inspection retains its exact captured basis. Logical programmes remain separate from Facilities and served areas.

Original configurations, including their originally recorded validity interval, remain unchanged. Additive succession determines the current projection.
...
The page guide `guide.eq.03` describes current controls. [Completion handover](../../../delivery/equipment-native-completion-handover.md) records executed checks, screenshots, failure comparisons and publication status. Source presence, functional evidence, visual comparison, business acceptance and deployment remain separate. No review fingerprint is asserted.

## Implementation verification

Implementation checkpoint `cad07b414d1cd2deab95ce8d2b459e01ddbed8ab` was inspected on 24 September 2026. These are original synthetic runtime captures, separate from accepted design images: [eq-configuration-1440.png](../../../testing/evidence/equipment-native/eq-configuration-1440.png), [eq-configuration-390.png](../../../testing/evidence/equipment-native/eq-configuration-390.png).
