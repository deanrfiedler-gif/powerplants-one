...
(() => {
  ...
  function command(state, actor, expected, action, payload = {}, at = new Date().toISOString()) {
    ...
    if (action === 'propose' || action === 'edit') {
      ...
      result = { id: `SYN-EST-2407-D${4 + next.successors.length}`, revision: 4 + next.successors.length, predecessor: 'SYN-EST-2407/r3', created: at, actor, reason: payload.reason.trim(), status: 'Draft · needs cost review', selected: preview.selected, lines: preview.lines, total: preview.after, policy: clone(policy), previewFingerprint: preview.fingerprint, issuedQuotation: clone(ESTIMATE.issuedBasis), review: 'No inherited approval', unresolved: preview.lines.flatMap(l => l.source ? blockers(l.source,l).map(reason => `${l.description}: ${reason}.`) : l.manual.review === 'Reviewed' ? [] : [`${l.description}: manual allowance needs confirmation.`]).concat('Import duty and complete landed-cost policy remain unconfirmed.') };
