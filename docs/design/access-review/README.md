...
| `capabilities.js` | **Generated. Do not edit.** The current `Capability` union from `src/platform/permissions.ts` and the 31-value hosted tester set from `scripts/demo-database.ts`, with the SHA-256 of both sources. |
