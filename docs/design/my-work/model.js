(function(root){
  ...
  function allItems(s){
    ...
    items.push({...base,id:'qa-review:'+sub.id,title:`Review inspection r${String(sub.revision).padStart(2,'0')} · Northbank`,type:'Review',owner:people[1],due:TODAY,status:review&&['Accept evidence','Return for correction'].includes(review.decision)?'Completed':'Open',reason:`${sub.outcome} evidence from ${sub.actor}; the exact submission needs an Assurance decision.`,qa:'review',submissionId:sub.id});
    for(const d of p.defects)items.push({...base,id:'qa-defect:'+d.id,title:'Correct and retest the pressure finding',type:'Correction',owner:d.owner,due:d.due,status:d.status==='Closed'?'Completed':d.status==='Correction recorded'?'InProgress':'Open',reason:`Original ${d.checkId} finding remains linked to ${d.submissions.length} submission(s).`,qa:'defect',defectId:d.id});
    if(review?.decision==='Accept evidence')items.push({...base,id:'qa-release:'+sub.id,title:'Decide the fertigation release boundary',type:'Release',owner:people[1],due:TODAY,status:release?'Completed':'Open',reason:'Evidence acceptance is recorded; a separate scope release decision is still required.',qa:'release',submissionId:sub.id});
