...
| `workspace.css` | r20 token core, reused by name from the AD-01 r01 source, scoped under `#ppo-contacts`, with the register, stakeholder, reliance, panel and phone treatments. |
...
| `source-pins.json` | Reused-asset hashes, the base commit, the theme edition, and the content hash of every pinned contract source. |
