...
2. For this first release of the merged Leads/Projects app, choose **upgrade-and-deploy**. The [bounded upgrade](../decisions/azure-existing-demo-upgrade.md) requires the existing demo baseline through 17 and applies missing Leads 18 / Projects 19 plus additive, expiry-preserving tester grants. It preserves saved records and current invitation state. A conflicting original Gantt-18 ledger is refused; do not reset or rewrite its receipts. Later image updates can use **deploy**, which verifies the database without migrating it. Future migrations need a reviewed extension of this bounded procedure.
3. Open the same workflow, choose **main** and the chosen operation. It builds the selected run's full source commit for Linux/amd64, with a source label. Tags include source SHA, run ID and attempt so repeated runs do not overwrite the same tag. Docker build/push failures stop before any app update.
4. The workflow resolves the pushed image to a validated SHA-256 digest, selects it on the existing operator job and runs `upgrade` or `verify`. It retains the operator's existing secrets and checks its epoch/tenant against the web app. It waits for that exact execution to succeed before updating the web app. It then waits for the selected web revision to be ready, healthy and using that digest; an old healthy revision cannot pass. Finally it configures the worker with the same digest and verifies the saved worker image.
...
Build, push or operator failure leaves web/worker images unchanged. A failed database transaction rolls back the upgrade together with its grant changes. If the operator is still running, inspect that execution before retrying. A successful database upgrade remains installed if the later rollout fails. A web readiness failure does not advance the worker. A later worker or smoke-check failure can leave a partial update; the summary explicitly says deployment did not complete. Inspect both actual images and the failed step before retrying. No automatic rollback is attempted and existing scheduled worker executions may finish using the old image.
