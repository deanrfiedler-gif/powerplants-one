...
**Owner:** Dean Fiedler · **Scope:** personal private synthetic prototype · **Status:** implementation and verification in progress; not yet published.
...
Original artifact archives and a per-file hash/provenance manifest will be linked from the maintained evidence record after executed visual inspection. Review-format copies are separate from the executed checkout and must not be confused with source or screenshots. Failed screenshots are retained with their rejection/fix, not silently replaced. Visual evidence supplements original receipts, HTTP retrieval and PostgreSQL facts.
