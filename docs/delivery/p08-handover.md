...
| 34007731421 | 9 unit, 160 PostgreSQL, 13 HTTP, all process-restart phases and 53/54 browser cases passed | A delayed BroadcastChannel lock notification could hide a newer verified owner's view during identity return. Re-read committed ownership for both lock/refresh notifications, and clear the old view before switching. The browser procedure now requires each exact online identity before its isolation/restoration assertions; all eight original rows and unchanged payloads remain required. No cross-owner evidence exposure was observed. |
