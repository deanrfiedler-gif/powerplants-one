...
Review order: [server contracts PR #285](https://github.com/deanrfiedler-gif/powerplants-one/pull/285), branch `feat/cs-customer-location-contracts`, head `51b4787`; then native branch `feat/cs-customer-location-completion` with `f682204` (Customer/Contact/Stakeholder/Site) and `36035a309db04000c703386e5edf5b3815de2daa` (durable receiving workflows and proof), followed by the evidence/guide commit. The dependent PR is based on the contracts branch so reviewers see the native increment separately.
