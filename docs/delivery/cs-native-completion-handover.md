...
The later concurrent PR #286 (component catalogue) remained open at the publication preparation checkpoint, head `b30df175`. Its published file list overlaps the shell, development register/guides and AGENTS guidance. No unfinished code was copied. Main was fetched again and remained `25170bf`; `git merge-base --is-ancestor origin/main HEAD` succeeds. This incorporates the final merged #284 state. A later merged contribution must still be reconciled before merge.
...
Review order: [server contracts PR #285](https://github.com/deanrfiedler-gif/powerplants-one/pull/285), branch `feat/cs-customer-location-contracts`, head `51b4787`; then [native PR #287](https://github.com/deanrfiedler-gif/powerplants-one/pull/287), branch `feat/cs-customer-location-completion`, with `f682204` (Customer/Contact/Stakeholder/Site), `36035a309db04000c703386e5edf5b3815de2daa` (durable receiving workflows and proof) and `94e7d05` (guides/evidence), followed by this documentation-only publication-link successor. The dependent PR is based on the contracts branch so reviewers see the native increment separately. Both PRs remain unmerged; remote checks are available on their exact heads.
