...
Post-rebase lint/typecheck/build, 21 focused units, original Finance HTTP isolation and all documentation/register integrity checks passed. The combined local browser run retains two disclosed timeouts; the live PR records isolated retries and final exact-head CI. Three additional inspected shell/guide captures supplement the original 37. See the verification record for the exact limits; no incomplete local run is presented as all-green.

The isolated post-rebase retries passed 11/11 desktop My Work checks and 2/2 phone geometry checks, using unchanged deadlines and assertions. The earlier combined-run timeouts remain disclosed. Latest exact-head CI is recorded in PR #284.
