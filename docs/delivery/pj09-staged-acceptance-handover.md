...
Main advanced again to `8ed8b0c` during the repair. Its four conflicts are resolved with main's semantic breadcrumb, shared header and register-heading mechanism retained. My Work/EN-06 use main's refined 240px grey menu. An explicit presentation option on the same shared menu primitive preserves EN-07/EN-08/PJ-09's 220px white menu, marker and captioned desktop trigger; preference keys, first-use defaults, overlay and keyboard behaviour are unchanged. The Projects acceptance entry and guarded lifecycle remain intact. No backend, schema, grants, receipts or output contracts changed in this reconciliation.

On compiled build `XHbFVS0D8oqyjF_KwMcJx`, the affected My Work/EN-06/EN-07/P11 browser suite passed 23 cases with 19 existing viewport exclusions and no failures. PJ-09's independent r22/flush-edge/negative-control/six-view check, native 200% zoom and EN-08's read-only six-view/phone check also passed. TypeScript, full ESLint and build passed. The [main #271 evidence](../testing/evidence/project-acceptance-r01/pr269-main271-2026-09-21/README.md) records exact commands, captures, setup failures and limits. These tests use isolated port 3014 and `ppo_synthetic_test`; the original review app at 3001 remains available and unchanged. Fresh remote CI and owner acceptance are separate.
