...
## PR #156 failed-check repair

Dean explicitly authorised repair of [PR #156](https://github.com/deanrfiedler-gif/powerplants-one/pull/156) on 14 September 2026. The original head was `044e69866634b65893d76c787f098bdda3ea146d`. Its failures were application defects, unrelated to the unavailable local logo attachment:

- [CRM database job 103786997369](https://github.com/deanrfiedler-gif/powerplants-one/actions/runs/34780675318/job/103786997369): SA-02 unknown-contact Discovery creation failed because the identification Activity foreign key was immediate. The pending migration 0022 now defers that exact composite relationship until commit; the owned, active, linked Activity evidence guard remains enforced.
- [Leads job 103786997364](https://github.com/deanrfiedler-gif/powerplants-one/actions/runs/34780675355/job/103786997364): a missing identification action reached the row constraint before the specific command validation. Validation now precedes insertion. The rollback regression also supplies an unavailable identification UUID to prove that a later failure leaves no tentative opportunity, Activity or conversion effects.
- [Compiled browser job 103786997466](https://github.com/deanrfiedler-gif/powerplants-one/actions/runs/34780675376/job/103786997466): both sales journeys reached a disabled Save next action button after a successful stage change. An accepted stage receipt now advances a clean sibling form's version. Independently edited proposals retain their input and original version for deliberate comparison; busy or uncertain saves prevent another stage command.

These corrections and their regression cases are backported from integration PR #157, source `503473dc136046812fbd42907aca448d7e895b9d`. The related phone corrections retain wrapping stage labels, visible returned keyboard focus, stable scroll restoration and usable 390/320 px feedback. The browser assertions require the focused control to be fully in the viewport. Original assertions, retries and timeouts are preserved.

This repair updates #156's existing source branch only. #157 remains the combined integration and merge surface; its unrelated changes are not imported here. No migration 0022 has been deployed by this repair. Applicable checks on the repaired #156 source must be recorded in its PR before declaring CI resolved; #157's earlier passes are supporting evidence, not a substitute for this branch's results.

Repair verification: local lint, TypeScript, all 85 unit tests, production build, the three Python documentation checks and `git diff --check` passed. This checkout used the available Node 24.19.0/npm 11.9.0; the reused pinned runtime executable could not start, so pinned-runtime verification remains with CI. An initial build refused an external `node_modules` symlink; using a private dependency copy resolved that environment error without changing application configuration. PostgreSQL/browser execution remains with the disposable GitHub jobs. All nine changed application, migration and test blobs were compared against #157's exact published source and match.
