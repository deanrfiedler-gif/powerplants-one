...
**Document ID:** PPO-DEMO-INTEGRATION · **Revision:** r03 · **Date:** 10 September 2026 · **Owner:** Dean Fiedler · **State:** Main reconciliation prepared; final-source CI/review/merge tracked in PR #73; cloud acceptance separate.
...
## 10 September — main reconciliation

Dean requested the first step of the integration order, PR #73. Fresh GitHub inspection confirmed main `3c155c71bed214298627c108f5f52b5f916096ff`, integration head `93828dd898b55af8b8d2361983a8f554160d369d` and common ancestor `f8035b5c55251da4da52430adf2f83094feccd6b`. Main has one additional commit: approved CRM scope #83. The integration head has eighteen additional commits. All eight workflows on that exact integration head completed successfully, including [Application assurance](https://github.com/deanrfiedler-gif/powerplants-one/actions/runs/34336965713), [CRM assurance](https://github.com/deanrfiedler-gif/powerplants-one/actions/runs/34336965880) and [Azure preparation](https://github.com/deanrfiedler-gif/powerplants-one/actions/runs/34336965884). These supersede the earlier pending CI notes for that source, but are not tests of this later reconciliation.

The merge produced two documentation conflicts. The document register now retains the approved CRM scope/report and current blueprint revisions alongside the newer Azure connection, runtime and CRM integration records. Project instructions retain the approved scope, private-mailbox controls and current workflow boundaries. BP-03 retains both main's accepted scope and the branch's implementation supplement. README points to working master r05. Both original histories are preserved; no runtime, migration, fixture, test, dependency, asset, infrastructure or workflow file changes relative to the integration parent.

Local foundation and prototype checks passed immediately after reconciliation. Naming initially rejected 8,177 characters in the combined project instructions; two wording reductions retained the controls and reduced the file to 7,984 characters without altering the 8,000-character validator. All three documentation checks then passed. All sixteen Python operator tests passed using mocked Azure calls; their synthetic deployment messages do not represent a cloud deployment. Issued-source hashes, the 78 parent requirements, register uniqueness and preservation of the approved scope are checked before publication.

The [PR #73 publication record](https://github.com/deanrfiedler-gif/powerplants-one/pull/73) records the final commit/tree, current-source workflows, any CI hold, screenshot review and actual merge outcome. New Node/browser/database execution is not claimed by these local documentation and Python checks. The installed local Node/npm differ from the repository pins, so runtime acceptance uses the unchanged pinned CI definitions. Merge requires the current source checks and review; no failing or pending run is treated as accepted. Live Microsoft sign-in, HTTPS, physical-device acceptance and hosted-image updates remain separate.

After verified merge, the next ordered contribution is CRM PR #75, followed by shell #84 and login #86. Reconcile each with accepted main and check its own resulting source; this task does not merge or rewrite those branches.
