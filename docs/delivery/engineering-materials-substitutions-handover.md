...
4. Open **`http://127.0.0.1:3000/engineering/materials`**, or from Engineering choose *Released materials & substitutions*. It opens the workspace on the package you last opened in this browser, otherwise the first one with a material set; change package from the *Package / project* control in the context row, whose last entry, *Choose another package…*, is the searchable list. The demonstration package is `/engineering/6460fa8c-ba7f-4830-897b-29592943ebe4/materials`.
...
|---|---|
| **Materials register** | Composition follows the owner's refined image of 20 September (design record 5.1). Flush register of the set; All / Ready for review / Needs attention with server counts; search, discipline, Add condition (owner, mapping, readiness, alternate), sort, columns, paging; every criterion in the URL. Opening a line inspects it without ticking it; ticks are a separate selection with an indeterminate page checkbox. Inspector: specified and proposed product, compatibility evidence, next action and review due, *Material required-by* with "Date needed" when unknown, exact source with purpose and currentness, and technical acceptance, material release and supply handover shown apart; *View material details* holds the reasons, quantities, areas served and the correct and remove actions. Add, correct and remove a requirement; CSV export |
...
- **Local ledger, updated on Dean's decision, 20 September.** `public.ppo_migrations` held version 29 under the first checksum (`c9484a1e75d1…`), so `npm run db:migrate` answered "Migration checksum mismatch" on `main` and on every branch taken from it. The update was held until the EN-07 branch carried the corrected file; once it did, the recommendation was put to Dean, who answered: "Proceed however you believe is the most professional." The row was then updated in one guarded transaction: `UPDATE public.ppo_migrations SET sha256='a4f348864915c24262a2819cf9da33bb5171f7e04bdb7b1e04f045017d41d2b4' WHERE version=29 AND sha256='c9484a1e75d1425117604b0b730017c2e17e2f9b0b9be4e72f72196c5e333d8a';`, required to change exactly one row, with the other 28 ledger rows compared before and after and found identical. No schema, data or other ledger row was touched. A read-only comparison afterwards hashed every registered migration file in the main checkout as the runner does: 28 of 28 match the ledger and none is unapplied, so `db:migrate` there has nothing to refuse or apply. `npm run db:migrate` itself was not run afterwards, because the session's permission system declined it; run it once to see the same result. Unchanged and unrelated: a fresh LF worktree still mismatches on 0001–0027, because this database recorded those from CRLF working copies.
