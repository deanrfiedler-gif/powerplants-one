...
Current-main reconciliation: main advanced first to `6c5e7c4`, then to `9e49a57` with Service Job Pack PR #304. Merge `b28242c` incorporates that current main without conflicts; the shared register/guide/component changes are retained and Sales runtime files are unchanged by the merge. PR #301 adds `/estimating` workload without a frozen Sales intake aggregate. PR #302 owns migration 0045 and ADR-0045. Sales therefore uses migration 0046 and ADR-0046; no files in either other worktree are modified. Subsequent merges still require normal register, guide, receipt and migration reconciliation.
...
The merged compiled application also passed a direct native Sales restart comparison: five handovers, two aftercare records, 48 immutable events and original receipts, two Won obligations and 30 shared Activities survived actual application/PostgreSQL restarts unchanged. Accepted Estimating/Won and Closed aftercare records were read through authorised HTTP on both sides. [Retained hashes and counts](../testing/evidence/sales-native-completion/restart-verification.json) record that evidence separately from screenshots and CI.

Latest completed Sales CI at `69c7427`: 388/388 units, 62/62 CRM/Sales database cases, 31 browser passes with two existing project skips, I2 17/17 and actual application/PostgreSQL restart verification. Its full compiled-application browser lane also passed: 374 passes and 61 existing project skips. The broad application/database/HTTP jobs remain independently visible in the PR; their completion is not inferred from the focused Sales result. After incorporating `9e49a57`, the local production build and register check passed again (280 entries, 126 routes, 25 component records, no integrity errors).

The separate broad application browser attempt passed 373, skipped 61 and failed one generic error-selector assertion during actual CRM permission revocation. Commit `5c0493b` targets the authoritative CRM refusal while retaining every private-content removal, late-response and unchanged-Activity assertion. The focused compiled desktop/phone replay passed 2/2. Full head checks remain the merge gate; earlier attempts are not relabelled as successful.
...
- Commits: `dc9cedf` persistence; `b836205` native pages and insights; `146368f` tests/guides; `410e6f6` retained navigation; `575ae00` phone containment and saved-revision readiness; `23fcb3a` paired layout review and receiving assurance; `69c7427` isolated HTTP source and worklist readiness; `60a1dcf` visual evidence; `b28242c` current-main reconciliation; `5c0493b` authoritative revocation assertion. Final evidence/documentation publication follows these commits in Git history.
- Branch: `feature/sales-cr01-cr05-completion`. Worktree: `C:/Users/Dean.Fiedler/Projects/powerplants-one/tmp/sales-cr01-cr05-completion`. Base reconciled through `9e49a57332aacfbb45a43fdd4d17c1d031fc80fb` in merge `b28242c3b67b810efd3730fc1c317a0b5da7fabb`. The original checkout and other domain worktrees remain untouched.
