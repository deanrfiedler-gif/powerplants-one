...
**State:** Native implementation and bounded local verification complete; open as [draft PR #274](https://github.com/deanrfiedler-gif/powerplants-one/pull/274). Owner acceptance and deployment remain separate. **Branch:** `feat/cs05-facilities-growing-areas`. **Original code candidate:** `5f6e683e7c809e1850e932ed684b9dd3725d13d0`; code tree `6dde740a7d3a017fff68ce7644893e02081e92f5`. **Original source:** main `af3f045`. Dean authorised execution of the supplied r02 documents. Dean now authorises merge after successful checks; deployment and operational writes remain outside this repair.
...
## PR #274 integration and error repair

The current branch combines ES-08 `0148514` with merged ES-02 main `460cf0b`, then retains CS-05 migration 0041 and its original SQL/seed bytes. All exact migration/seed consumers, hosted-upgrade review, shared module routing and 39 Facility CSS selector additions are reconciled. The [ADR integration record](../decisions/ADR-0037-cs05-facilities-native.md#combined-source-assurance-22-september-2026) explains the counter/identity assertion repair, Contacts source review and 21-destination menu check.

The two original CRM upgrade failures came from legitimate additive seed rows/counter advances and ambiguous counter ordering. Their corrections assert all old identities, the exact 16 new identity rows and only the three authorised counter increments, then retain repeat-seed checks. No production assertion, capability, workflow case or original operation is removed. The current source and final validation/merge results are maintained on PR #274; the original 5f6e683 captures and partial acceptance dispositions remain historical evidence with their original scope.

Local repair validation passes TypeScript, full lint, production build, actionlint, foundation/naming (78 parent IDs), 16 focused units, both previously failing CRM upgrade cases and all ten Facility database groups on an isolated `ppo_synthetic_test` service at loopback port 5544. All 14 desktop/phone Facility cases pass in one run against the rebuilt application at port 3041; 1440px and 320px captures were inspected. Contacts passes 103 model and 62 browser groups with the issued HTML unchanged. The CRM component run passed 36 cases before the stale menu assertion; the corrected menu case then passed separately. Full combined-source CI remains the single-run proof for that suite.

The first local upgrade attempt stopped in document setup because Windows returned an 8.3 short path that differed from the canonical directory; using the canonical external temporary directory resolves that environment refusal without changing document-store guards. An additional early-version identity comparison was expanded to retain six existing CRM/Finance/Email seed identities as well as CS-05's exact sixteen additions. Original failed outputs remain local review evidence; final CI artifacts and outcomes remain on the PR.
