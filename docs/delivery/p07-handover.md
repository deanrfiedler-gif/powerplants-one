...
Each technician starts only their own attendance. The first sets appointment InProgress and the authoritative actual-start instant; another crew member must use current versions and repeat the full guard. Booking duration and acknowledgement create no labour. Current P06 authority, complete exact issue/hash acknowledgements, scope, assignment/schedule, non-waivable controls, competency and exact original file retrieval are checked inside the shared transaction with resource locks. Snapshot hashes retain the actual actor, controls, exact competency references and received time. The resource must also be effective now, even if a future booking and its pack are valid; an added real PostgreSQL case publishes a separate immutable future-effective synthetic crew bundle and proves current start refusal without changing old source records. Current derived component readiness is never treated as a transferable credential.
