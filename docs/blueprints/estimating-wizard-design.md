...
Local foundation, prototype, naming, existing estimating-design arithmetic, new wizard-model and whitespace checks passed. The first [CI design run](https://github.com/deanrfiedler-gif/powerplants-one/actions/runs/34347239009) at `a46c5cac` passed the existing design checks and reached the new manual-adjustment selector after baseline and labour-edit interactions. It failed that exact-label lookup; `0bc90126` corrects the three select accessible names without weakening assertions or deadlines. The downloaded initial artifact SHA-256 is `d3e4fa12b57161f8e5bd7b78fb3cd09a3e7a069741bc63e9d10ef1f688b7d9fa`. Its requirements, proposal and adjusted-estimate desktop captures were visually inspected: branding, line amounts, controls and wrapping were legible. Phone, rerun and quotation rendered review remain pending corrected-head CI. The original run does not establish full wizard-browser acceptance.
