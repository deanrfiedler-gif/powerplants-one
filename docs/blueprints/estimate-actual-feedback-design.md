...
**The idea is sound and worth building. The value is not where it first appears to be, and two of its assumptions do not currently hold.**

Three findings drive everything below.

| Finding | Consequence |
|---|---|
| **The high-value starting point is Service, not Projects.** Projects produce perhaps single-digit major greenhouse builds a year. Service and parts produce hundreds of jobs. Learning needs repetition, and PPO already captures service actuals — P07/P09 field labour, travel and parts, reviewed at P09, dispositioned at P10 — while Projects captures schedule only. | Build the loop where the data already exists and the sample is large enough to mean something. Extend it to Projects after the mechanism is proven. |
...
Two assumptions in the proposal as put do not currently hold:
...
|---|---|---|---|
| P1 | **Commercial lineage.** An estimate version and quote revision reachable from the delivered work order or project. | Absent. `ppo.projects` has no opportunity, estimate or quote foreign key. Service work orders have no estimate link. | Everything |
...
|---|---|---|---|
| **F1 — Service close-out record** | Structured close-out at service report review: declared basis, breakdown-coded estimated versus delivered, reason codes, narrative, evidence links. Manual delivered-side entry. | P1 (service scope), P3, P4 | No ERP integration. Highest value per unit of work, because Service has the volume and PPO already holds the reviewed actuals. |
...
Projects coverage arrives by extending F1's delivery target once FD-05 is defined and a project cost read is authorised — not as a separate parallel build.
...
|---|---|---|
| 1 | Start in Service rather than Projects | Reverses the premise as put. Everything below assumes the answer. |
...
| 5 | Materiality threshold requiring attribution | Too low and close-out becomes clerical; too high and the learning is lost |
