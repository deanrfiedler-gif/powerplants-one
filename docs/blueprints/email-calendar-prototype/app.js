...
const icons = {
  ...
  record: '<rect x="4" y="4" width="16" height="17" rx="2"/><path d="M8 3v3h8V3M8 11h8M8 16h5"/>',
  settings:
...
const canRecord = (id) => {
  ...
  return !!r && r.readers.includes(state.actor) && !(state.actor === "jordan" && state.revoked);
};
...
const visibleMessages = (t) => (isOwner() ? t.messages : t.messages.filter((m) => canShare(t, m)));
...
const visibleLinks = (t) =>
  t.links.filter(
    (id) =>
      canRecord(id) &&
      (isOwner() ||
        state.shared.some((s) => s.thread === t.id && s.record === id && s.actor === state.actor)),
  );
const isShared = (t, m) => state.shared.some((s) => s.thread === t.id && s.message === m.id);
...
function mailResults() {
  const rows = visibleThreads()
    ...
    .filter((t) => {
      ...
      const terms = [t.subject, t.from, ...ms.map((m) => m.body)].join(" ").toLowerCase();
...
function calendar() {
  if (["disconnected", "expired"].includes(state.sync))
    return `<main id="main" tabindex="-1" class="page">${empty(state.sync === "expired" ? "Reconnect required" : "Calendar disconnected", "Reconnect the sample mailbox from Settings.")}</main>`;
  return `<main id="main" tabindex="-1" class="page"><div class="page-header"><div><h2>${isOwner() ? "Your agenda" : "Shared availability"}</h2><p class="muted">Brisbane · AEST (UTC+10)</p></div><label>Day<select id="agenda-day"><option value="2026-09-08" ${state.day === "2026-09-08" ? "selected" : ""}>Tuesday, 8 Sept 2026</option><option value="2026-09-09" ${state.day === "2026-09-09" ? "selected" : ""}>Wednesday, 9 Sept 2026</option></select></label></div>${state.sync !== "ready" ? '<div class="callout warning">Sample sync is not current. This agenda cannot confirm availability.</div><div class="spacer"></div>' : ""}<div class="calendar-layout"><section class="agenda"><div class="agenda-day">${state.day === "2026-09-08" ? "Tuesday, 8 September 2026" : "Wednesday, 9 September 2026"}</div>${agendaEvents()}</section><aside class="panel"><h3>One view of your day</h3><div class="stack"><p class="small muted">Outlook meetings and PPO follow-ups keep their own source and meaning.</p>${badge("Outlook · Read only", "calendar")}${badge("PPO Activity · Internal", "record", "green")}<div class="callout">A meeting does not authorise work, book a technician or complete a PPO Activity.</div><p class="small muted">Recurring meetings show an occurrence badge. Changes remain in Outlook during the first pilot.</p></div></aside></div><p class="prototype-note">Fictional events. Shared free/busy is a design scenario; it is excluded from the first Microsoft pilot.</p></main>`;
}
function allEvents() {
  return [
    ...EC_FIXTURES.events,
...
function agendaEvents() {
  let events = allEvents().filter((e) => e.day === state.day);
  if (!isOwner())
    events =
      state.privateBusy && !state.revoked ? events.filter((e) => e.source === "Outlook") : [];
  if (!events.length)
    return empty(
      "No events to show",
      isOwner()
        ? "No sample events for this day."
        : "No availability has been shared with this preview account.",
    );
  return events
    .map((e) => {
      const hidden = !isOwner();
      return button(
        `<span class="event-time">${e.start}<br><span>${e.end}</span></span><span class="event-content ${hidden || e.private ? "private" : e.source === "PPO Activity" ? "ppo" : ""}"><h3>${esc(hidden ? "Busy" : e.title)}</h3><p>${esc(hidden ? "Details are private" : e.with + " · " + e.location)}</p>${badge(hidden ? "Shared availability" : e.source, hidden ? "lock" : e.source === "Outlook" ? "calendar" : "record", e.source === "PPO Activity" ? "green" : "")}${!hidden && e.recurring ? " " + badge("Recurring occurrence", "refresh") : ""}${!hidden && e.private ? " " + badge("Private", "lock") : ""}</span>`,
        "event",
        `data-event="${e.id}"`,
        "event",
      );
...
function settings() {
  return `<main id="main" tabindex="-1" class="page narrow"><div class="page-header"><div><h2>Email & Calendar settings</h2><p class="muted">Connection, privacy and sync preferences</p></div>${button("First Microsoft pilot", "nav", 'data-view="pilot"')}</div><div class="settings-grid"><section class="panel"><h3>Outlook connection</h3><p class="small muted">alex@ppo.example · Fictional Microsoft 365 mailbox</p><div class="setting"><div class="grow"><strong>Connection</strong><p>No Microsoft account is connected.</p></div>${badge("Simulated", "calendar", "navy")}</div><div class="setting"><div class="grow"><strong>Email folders</strong><p>Inbox and Sent Items</p></div>${badge("Read only", "lock")}</div><div class="setting"><div class="grow"><strong>Calendar</strong><p>Primary calendar · 30 days before to 90 days ahead in the proposed pilot</p></div>${badge("Read only", "lock")}</div><div class="setting"><div class="grow"><strong>Outlook changes</strong><p>Send, archive, delete, mark read and meeting updates are unavailable in this stage.</p></div></div></section><section class="panel"><h3>Privacy defaults</h3><div class="setting"><div class="grow"><strong>New email</strong><p>Private to the mailbox owner.</p></div>${icon("lock")}</div><div class="setting"><div class="grow"><strong>Record matching</strong><p>Suggest possible records. A person chooses the link.</p></div></div><div class="setting"><div class="grow"><strong>Sharing</strong><p>Selected messages, one linked record and named eligible colleagues. Future replies stay private.</p></div></div><div class="setting"><div class="grow"><strong>Attachments</strong><p>Share separately with the selected message; default off. External images stay blocked.</p></div></div></section><section class="panel full"><h3>Prototype controls</h3><p class="small muted">These controls simulate states for design review. Changes last until you reload or reset this page.</p><div class="setting"><div class="grow"><label for="sync-state">Sample connection state</label><p>Show paused, expired, unavailable and disconnected experiences.</p></div><select id="sync-state">${[
    ...
    .map(([v, l]) => `<option value="${v}" ${state.sync === v ? "selected" : ""}>${l}</option>`)
    .join(
      "",
...
function render() {
  app.innerHTML = shell(({ email: mail, record, calendar, settings, pilot }[state.view] || mail)());
}
...
function linkDialog() {
  ...
  openDialog(
    ...
    button("Cancel", "close") + button("Save links", "save-links", "", "primary"),
  );
}
function shareDialog(messageId) {
  ...
  const existing = state.shared.filter((s) => s.thread === t.id && s.message === m.id);
  if (!links.length) {
    openDialog(
  ...
  openDialog(
    ...
    (existing.length
      ? button("Withdraw sharing", "withdraw-share", `data-message="${m.id}"`, "danger")
      : button("Cancel", "close")) + button("Share selected message", "save-share", "", "primary"),
  );
}
function followupDialog() {
  ...
  if (!t.links.length) {
    openDialog(
      ...
      button("Close", "close") + button("Choose a record", "followup-link", "", "primary"),
    );
    return;
  }
  openDialog(
    ...
    `<div class="callout">This creates an internal PPO Activity. It does not create an Outlook event or send an invitation.</div><label>Follow-up action<input id="followup-title" required maxlength="160" placeholder="For example, confirm site visit details"></label><label>Related record<select id="followup-record">${t.links
      ...
      .map((id) => `<option value="${id}">${esc(recordById(id).title)}</option>`)
      .join(
        "",
    ...
    button("Cancel", "close") + button("Create follow-up", "save-followup", "", "primary"),
  );
}
function eventDialog(id) {
  ...
  if (!e) return;
  if (!isOwner()) {
    if (state.revoked || !state.privateBusy) return;
    openDialog(
      "Busy",
...
document.addEventListener("click", (e) => {
  ...
  if (a === "refresh") {
    if (state.sync === "ready") toast("Sample refresh complete. No Microsoft request was made.");
    else
      toast(
  ...
  if (a === "save-links") {
    const t = currentThread();
    if (!isOwner() || !t) return;
    const links = [...dialog.querySelectorAll('input[name="record-link"]:checked')]
      .map((x) => x.value)
      .filter(canRecord);
    t.links = links;
    state.shared = state.shared.filter((s) => s.thread !== t.id || links.includes(s.record));
    closeDialog();
    render();
  ...
  if (a === "save-share") {
    ...
    if (state.revoked || !t.links.includes(rid) || !recordById(rid)?.readers.includes("jordan")) {
    ...
    closeDialog();
    render();
    toast("Selected message shared with Jordan in this demo. Future replies remain private.");
  }
  if (a === "withdraw-share") {
    const t = currentThread();
    if (!isOwner() || !t) return;
  ...
  if (a === "save-followup") {
    ...
    if (
      ...
      error.textContent = "Enter an action, related record and valid due date and time.";
      dialog
        .querySelector(!title ? "#followup-title" : !rid ? "#followup-record" : "#followup-due")
        .focus();
      return;
    }
    if (t.followup) {
      error.textContent = "This conversation already has a follow-up in the demo.";
      return;
    }
  ...
  if (a === "reset") {
    openDialog(
      ...
      button("Cancel", "close") + button("Reset demo", "confirm-reset", "", "primary"),
    );
  }
  if (a === "confirm-reset") {
