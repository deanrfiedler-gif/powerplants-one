...
## 19 September — hosted booking workflow access

Dean authorised the next step after the hosted planner repair: enable and exercise the existing proposal, customer confirmation, crew booking and rescheduling workflow. PL-01 remains complete. This is access to the existing synthetic workflow, not a replacement scheduling feature.

The bounded invited-tester set adds `service.work_order.edit`, `service.readiness.assess`, `schedule.contact` and `schedule.manage`. Readiness assessment is necessary because every new proposal requires its own reviewed ToolPreparation evidence before confirmation. The existing capabilities also expose their existing work-scope editing, readiness-review and future cancellation actions; no new capability semantics or bypass is introduced. Scope authorisation, change-request authority, dispatch/pack issue, field and Finance access remain absent. Customer contact records remain simulated or manual records, not outbound communications.
