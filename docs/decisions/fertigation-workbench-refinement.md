...
| A3 | **Gap, open.** After A1, the r02 sample previews but is **held**: 80 populated legacy fields across 21 record families have no native mapping. Confirm is disabled and the screen offers no way to resolve them, so the exemplar r02 project cannot reach a native draft | `interchange.ts` refuses a held preview ("Resolve every listed mapping before creating a native revision"); `fertigation-import.tsx` disables Confirm while held | F1 |
| A4 | **Gap, open.** Once imported, the draft would open with 28 findings (21 incomplete, 7 review). They reduce to 12 declarations: 4 fully answerable from values already in the r02 file (7 findings), 6 partly (12 findings) and 2 needing new input (9 findings) | Native engine output for the r02 sample | F1 |
...
6. F1: raise an ADR for explicit placement of held legacy fields before any code.
