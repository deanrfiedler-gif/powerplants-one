...
**Status:** Direction given; implementation in progress. On 23 September 2026 Dean directed that the improvements be applied (see [Direction and implementation](#direction-and-implementation-23-september-2026)). D1 and D7, features F2–F7 and F9, and the placement part of F1 ([ADR-0044](ADR-0044-fertigation-held-import-placement.md)) are implemented on this branch; the rest remain proposed. Visual review, owner acceptance of the running screens and CI are separate and not yet recorded.
**Owner:** Dean Fiedler.
...
| A4 | **Gap, open.** Once imported (now possible, A3), the draft would open with 28 findings (21 incomplete, 7 review). They reduce to 12 declarations: 4 fully answerable from values already in the r02 file (7 findings), 6 partly (12 findings) and 2 needing new input (9 findings) | Native engine output for the r02 sample | F1 |
...
Not built yet: the rest of F1 (the "evidence for a declaration" and "mapped to a native field" dispositions, and offering r02 values for the 12 declarations of A4), F8 (site-visit capture), F10 and F11 (new calculations need a new calculation edition), F12, and A9 (a required save reason would change three existing save journeys).
...
6. F1: done for placement; [ADR-0044](ADR-0044-fertigation-held-import-placement.md) was raised before the code. Per-family native mappings and declaration offers need their own rule and test each.
