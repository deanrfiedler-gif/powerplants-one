...
[ES-01 Estimating intake and workload design board](es01-design-board.md) records the user's 24 September 2026 decision that ES-01 design uses the Powerplants One design system, not the separate PHYTO system. Its proposals P1–P9 (register with a persistent detail panel, readiness counts that need a read-model addition, readiness tones, drawer below 1360 CSS px and others) and open questions O1–O2 are **not accepted**; five findings against the running build (B1–B5) are recorded from source. The board has [retained captures](../reference/ui/estimating/workload-design-board-r01/README.md). No implementation, interface change or UI baseline entry.
