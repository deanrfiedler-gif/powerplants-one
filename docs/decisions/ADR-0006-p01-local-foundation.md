...
Runtime/lint/test packages declare permissive MIT, ISC or Apache-2.0 licences; PostgreSQL uses the PostgreSQL licence. These are dependency licence observations, not an approval of corporate licensing or external services. Build/runtime tools remain separately licensed; preserve packaged notices on redistribution. No approved Powerplants logo or contact block was invented. Verdana/system fallback avoids a build-time remote font dependency.
