...
DP-22 was adopted on the same day, after I3 measured it. The r03 preparation action bar is sticky inside the module’s scroll container; on a phone a sticky box cannot be displaced above its containing block, and the form begins near the foot of the scrollport, so the bar was clamped to the form’s own top and fell 26 px (at 390 px wide) and 145 px (at 320 px) behind the application shell’s fixed navigation bar, obscuring the save and discard controls. The bar is now fixed above that navigation bar at ≤ 760 px, keeping r03’s box, padding, radius, shadow and 44 px targets and r03’s own static fallback below 650 px of viewport height. The bottom inset the plan anticipated does not reach the cause and was tested. This joins DP-11 and DP-12 as a departure forced by the application shell, which the standalone design was not built against.
...
The [application integration record](job-pack-integration.md) and [handover](../delivery/job-pack-integration-handover.md) record I5 completion evidence and the adopted departures. Implementation does not amend the accepted reference or grant owner/device acceptance.
