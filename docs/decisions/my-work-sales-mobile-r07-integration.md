...
| **Insights** | **Deferred.** | The application has no reporting module: no saved report, chart, dashboard, target or measured history, for Sales or anywhere else. Service reports (`/service/reports`) are a review workflow, not analysis. The tile opens an explanation of what insights will report on and promises nothing. **Missing:** a reporting surface, the measures themselves and their permission scoping. The Deals **Forecast** view (open opportunities by expected close month, unweighted) exists and is reached from Deals; Dean confirmed on 20 September that a forecast is **not** what Insights means, so it is not offered here as a substitute. |
...
| MP-11 | **Quick Action tiles show an icon alone**, five in a row at every phone width, where report r02 required four labelled tiles and two-by-two at narrow widths. | Dean's refinements 1 and 7. Each tile keeps its name for assistive technology and as a tooltip. A tile the identity cannot open is dimmed, dashed and marked with a small lock in place of the words "No access". Map uses the map-pin icon. At 320px each tile is still 53px wide and 64px tall, above the 44px target. |
| MP-12 | **A fifth tile, Insights**, sits fourth in the row, between Map and Tasks, where report r02 fixed the four tiles as Emails, Leads, Map, Tasks. | Dean's refinement 7, following the position in the reference he supplied. It opens an honest deferred state, as Map does. |
...
| M03 | Met in order and destination for the four r02 tiles, with Insights added fifth on Dean's instruction (MP-12); labels removed on his instruction (MP-11). Map and Insights open their deferred states; Emails is locked without `email.read` | Journeys 1, 3, 10 |
| M04 | Met | Journey 1 |
...
| 7 | Add an Insights tile to Quick Actions, with a graph icon like the supplied reference. | MP-12. A rising line inside an axis, drawn for the My Work set; the shell's own bar-chart `insights` mark belongs to the Deals view toggle and is untouched. Dean confirmed that Insights means reports and dashboards, not the Deals forecast, so the tile opens a deferred state rather than the forecast: see section 3. |
