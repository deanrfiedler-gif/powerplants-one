...
| First CI run on the pushed branch (run 35504309827, before r03 and the rebase) | 16 of 17 checks passed, among them the compiled browser suite with EN-07's four journeys, the Access review, demo, leads and Engineering intake suites and the E1 upgrade proof. The PostgreSQL proof ran 459 database tests and 458 passed: four of EN-07's five on their first run, and one failure, EN-07's own. **Cause:** the in-process helper the database suite signs in through reported 201 for every accepted command, while the prerequisites route is declared as creating nothing (`commandRoute(…, false)`) and answers 200. The test's expectation of 200 was the route's real answer; the helper was wrong, and now reports what each route sends. Because everything after that line had never run, the whole test was then replayed step for step over HTTP against the task-owned server, generated from the suite's own source: it passes, including the parallel confirmation race, the same-identity correction, the failed then passed retest and closure. Its two direct-SQL assertions (a closed change is immutable; attempts are append-only) cannot be replayed without the test database and rest on the rolled-back migration proof above until CI runs them |
| Second CI run, on head `b3c7908`: the r03 build, rebased, with the shifted dates (runs 35510260535 and 35510260556) | **Every EN-07 suite passed in CI.** All five tests of `tests/database/engineering-changes.test.ts`, including the one that failed before and its two direct-SQL assertions, which therefore no longer rest on the local proof alone; the EN-07 HTTP suite; and the three desktop and one phone journeys in the compiled application, with EN-06's five beside them. Fifteen of seventeen checks were green on the first attempt. The two red jobs failed only in My Work tests that depend on the time of day in Brisbane and that this change does not touch: `my-work.spec.ts:35` and `my-work-mobile.spec.ts:40` at 22:19 to 22:35 Brisbane, where the helper's `todaySlots()` refuses by design ("Too little of the Brisbane day is left to build today's scenario."), as it did on the documentation-only #267; and `MW-DB01` (`tests/database/my-work.test.ts:76`) at 23:56 Brisbane, where an appointment asked to start 60 minutes from now and end at the end of the local day starts after it ends, so the `starts_at` rule fires before the `due_date_only` rule the test expects. 458 of 459 database tests passed. Both jobs were rerun unchanged after the window closed, at about 06:05 Brisbane on 21 September. A lasting fix for those tests is a My Work test-design decision for Dean and is not made here |
