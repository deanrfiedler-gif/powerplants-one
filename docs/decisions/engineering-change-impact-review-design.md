...
4. **Merge order.** EN-06 (draft PR #265) first. Dean authorised publication on 20 September 2026 ("I authorise you to push to github"); the branch is pushed and open as draft PR #266 against `main`, because the Application workflows run only for pull requests into `main` and CI is the first run of the database suite. Until #265 merges its diff also shows EN-06's commits.
