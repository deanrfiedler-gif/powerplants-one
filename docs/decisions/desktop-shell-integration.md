...
At `081bbd46`, [the corrected component job](https://github.com/deanrfiedler-gif/powerplants-one/actions/runs/34426617572/job/102713113742) passed all nine applicable checks (six desktop-only cases are intentionally skipped in phone projects). This includes all four desktop viewport sizes and the retained phone/narrow-phone CRM checks. Screenshot artifacts were uploaded by CI, but their returned download links refused local retrieval with HTTP 403, so no manual screenshot inspection is claimed. The final follow-up replaces nested header/footer elements inside the top bar's popovers with equivalent styled title/footer containers. Full corrected-source assurance, manual visual review and deployment remain separate gates.

The corrected `081bbd46` database/CRM job also passed all five database cases, all four CRM edit/directory browser journeys and the application/database restart proof. Retained I2 passed 11 of 12 cases; the delayed-initial-identity case on phone found its identity selector had closed before selection. The added focus-out dismissal could close that form when a late page alert moved focus. The follow-up preserves the open selector on programmatic focus changes and retains explicit outside-click/Escape dismissal. The original I2 assertions and timeouts are unchanged and must pass on the follow-up source.
