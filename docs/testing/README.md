...
The [P10 handover](../delivery/p10-handover.md) separates exact-source/quantity/allocation, account arithmetic, processing and reconciliation components from full procedures and publication. Tests are `tests/unit/finance.test.ts`, `tests/database/finance*.test.ts`, `tests/http/finance.test.ts`, `tests/browser/finance.spec.ts` and `scripts/finance-restart-proof.ts`. The upgrade case issues actual P09 reports before applying additive migrations 0010/0011, repeats seed and compares immutable original rows and private bytes. The restart gate preserves a controlled claim, independently accepted unknown result and stored-but-unreleased original output through actual PostgreSQL/application/browser restarts. All earlier cases and restart gates remain required. Existing PT-06 and P09 full synthetic PT-15/PT-16 statuses remain; P10's final handover must record each additional full procedure only after every written precondition and step has executed. The authored AT/PT catalogues and 78 parent IDs are unchanged.
