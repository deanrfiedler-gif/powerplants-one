...
## Browser harness event ordering

Run [35153267345](https://github.com/deanrfiedler-gif/powerplants-one/actions/runs/35153267345) passed lint, model checks and the first 27 browser groups, then exposed a race between the queued dialog `close` handler and the following skip-link test. The harness now observes completion of `close` before beginning that separate interaction. Both focus-return and skip-target assertions remain intact; the generated HTML is unchanged by this harness correction. A fresh focused workflow verifies the corrected harness.
