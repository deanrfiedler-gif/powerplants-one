...
const origin = "http://127.0.0.1:" + (process.env.PPO_PORT ?? "3000"),
  out = "docs/testing/evidence/scheduling-refinement/images";
await mkdir(out, { recursive: true });
const profile = "tmp/scheduling-refinement-200-percent";
...
try {
  ...
  for (const [name, path, detail] of [
    ...
    [
      "field-team",
      "/service/technicians?day=2031-09-22&timezone=Australia%2FMelbourne",
      "",
    ],
    [
      "resource",
    ...
    if (name === "planner")
      await page
        .locator(".proposal-section .read-meta")
        .waitFor({ timeout: 30000 });
  ...
  await writeFile(
    "docs/testing/evidence/scheduling-refinement/zoom-metrics.json",
...
try {
  ...
  if (!response.ok()) throw Error("Synthetic identity failed");
  for (const [width, height] of [
    ...
    if (
      ...
      throw Error("Field Team overflow");
    await page.screenshot({
      path: out + "/field-team-" + width + "x" + height + ".png",
    });
  }
  ...
  for (const [name, path, detail] of [
    ...
    const target = detail
      ? page.getByRole("heading", { name: detail, exact: true })
      : page.locator(".scheduling-card").first();
    await target.waitFor({ timeout: 30000 });
    await target.evaluate((e) => e.scrollIntoView({ block: "start" }));
    await page.screenshot({ path: out + "/" + name + "-320x844-detail.png" });
  }
