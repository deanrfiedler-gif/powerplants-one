...
## CRM reload and Email/Calendar session boundaries

Head `b28bc71e` passed its compiled desktop/mobile suite. CRM job `106280604411` in run `35583235596` passed its 23 full-journey cases but one of the 17 I2 cases checked the opportunity heading five seconds after reload while the page still displayed “Loading permitted sales records…”. Artifact `10632050488` retains the inspected screenshot and failure context. The case now observes the real opportunity GET within the existing 15-second action budget, asserts HTTP 200 and `private, no-store`, then retains the original five-second heading assertion. Holding the actual detail response for 6.5 seconds reproduces the boundary on both viewports without replacing the body or status.

The independently observed Email/Calendar initial-session failure on PJ-09 `e8ad2df1` (run `35583990581`, job `106282970105`, artifact `10631264583`) has the same sequencing correction here: observe the actual initial GET, assert its expected unauthenticated 401 and private/no-store, then use the unchanged DOM assertion and real UI sign-in. Its 6.5-second real-response regression runs in the existing complete journey. Both temporary routes are removed after readiness. No application behavior, global assertion timeout, retry, migration or permission change is introduced. Fresh-head CI remains required.
