...
|---|---|---|---|
| BP-01 | Master Business & Build Blueprint | `docs/blueprints/BP-01-master-blueprint.md` | Canonical path implemented; working r03 naming amendment |
...
|---|---|---|
| `docs/standards/` | Project conventions and their registers | Added during naming adoption |
...
The supporting [document register](document-register.csv) and [exceptions register](naming-exceptions.csv) are implemented. The document register covers formally identified documents and planned blueprint reservations; ordinary supporting contracts retain their descriptive path identities.
...
The inspected r02-source service contract mixed `/service/work-orders/:id` reads with `/work-orders/:id/authorise` commands and similarly mixed ticket paths. ADR-0005 now consolidates the working API design under **`/service/work-orders`** and **`/service/tickets`**, retaining API-C/API-R IDs. This is implemented in the specification only; no endpoints or consumers exist yet.
...
### 20.1 Original migration schedule and current disposition

This table records the r02 migration design against the inspected commit. Its two path changes and active-reference repairs are implemented in this adoption edition. See [naming-adoption.md](naming-adoption.md) for completed items, contract mappings and future runtime obligations. Re-inventory current `main` before any subsequent migration.
