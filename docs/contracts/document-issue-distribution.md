...
**Status:** OUT-09/DAT-07/minimum DAT-11 implemented in P06; bounded DAT-09/OUT-10 in P09 and restricted OUT-14 in P10, with actual verification recorded separately. No actual SharePoint repository, template or retention policy has been verified in this package.
...
## P11 successor template and brand implementation

Seed 14 adds immutable version-2 definitions for OUT-09, OUT-10 and OUT-14 and advances only each checked original version-1 template policy. The transaction refuses an unexpected policy instead of overwriting newer work; repeated seeds preserve existing policy/grant/evidence rows. All version-1 definitions, renderer source bytes, issued bundles, presentation hashes, responses and provider identities remain unchanged and retrievable under current permission. Already reserved work still requires its original exact template/source recheck and cannot be released against changed policy. No original PDF is regenerated during recovery.

The successor definitions fingerprint the original rendering sources, frozen P11 rendering/definition sources and every exact font/logo/provenance asset. Future rendering changes require separate successor sources/definitions; do not edit issued version-2 rendering sources. The output embeds the intact accepted logo on navy with clear space and sufficient minimum size, plus a complete lossless WOFF font derived from the accepted Roboto TTF. The TTF and PNG remain unchanged. Local fontTools 4.61.1 conversion verified all 1,326 glyphs, 927 Unicode mappings, variable axes, horizontal metrics and every glyph's geometry against the source. The additional WOFF is 280,272 bytes; its checked provenance is `public/brand/Roboto-output-provenance.json`. No font subset or new runtime dependency is introduced.
