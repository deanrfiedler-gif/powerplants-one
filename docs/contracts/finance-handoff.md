...
Each handoff contains one work order, one exact company/customer/account context, exact report/entry revision IDs, mode, source evidence manifest, correlation ID and an accountable processing owner when claimed. PP-01 uses the separately verified synthetic account context described in the physical amendment below; operational ERP verification remains disabled. Separate handoffs handle different companies/currencies/billing bases. Multiple visits are permitted under the same work-order/account context with line-level attribution.
...
## P11 approved synthetic Travel amendment

[ADR-0018](../decisions/ADR-0018-p11-travel-and-integrated-quality.md) records Dean's 7 September 2026 approval. For the selected private P11 journey, exact whole-minute Travel is supported separately from Labour under successor Finance definition v2. Every Travel allocation requires explicit NonBillable disposition and a reason; billable quantity is zero and target group is absent. Full source conservation reserves/consumes the entire actual Travel quantity. Separate Finance review and no-posting reconciliation remain required. NonBillable means neither zero cost nor unpaid staff time.

This supersedes only the earlier Travel unsupported-treatment rule. Labour F-06, Break/Waiting/Other and fractional-minute blocks remain unchanged. Future billing or partial-billing treatment requires another maintained decision and successor policy preserving history. No operational payroll, expense, cost, rate, tax or charging policy is inferred. Migration 0013 and seed 13 preserve v1 and all source/issued bytes; stale unprocessed policy snapshots require review, while possible effects retain original lookup and cannot be replayed or cancelled. Actual implementation/verification is recorded in the P11 handover and external issue publication.
