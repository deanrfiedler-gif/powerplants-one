...
**Owner:** Dean Fiedler (`deanrfiedler-gif`) · **Stage:** P07 online field workflow component-verified; final publication evidence linked from the P07 handover · **Deployment:** none.
...
| `docs/testing/` | Full acceptance catalogue, P01–P07 component evidence and dependency inventory |
...
Application code is in `src/`, explicit SQL migrations and fixtures are in `db/`, and runtime checks are in `tests/`. See [P07 setup, verification and handover](docs/delivery/p07-handover.md) and [ADR-0006](docs/decisions/ADR-0006-p01-local-foundation.md) for the local-only implementation and limits.

## Run the local application

Follow the [exact P07 setup and run commands](docs/delivery/p07-handover.md#runtime-setup-and-recovery): Node 24.20.0, npm 11.19.0, PostgreSQL 16.15, `npm ci`, ignored local configuration, migration/seed and `npm run dev`. Open `http://127.0.0.1:3000`. Production startup is intentionally refused. The handover includes test, reset and recovery commands.
...
Technicians use `/my-jobs` and `/my-jobs/:id`. Riley and Morgan acknowledge and start independently. Supported fictional PNGs are registered, uploaded and verified before availability. Completion remains a draft. **Online workflow preview — offline/report/Finance work incomplete**.
