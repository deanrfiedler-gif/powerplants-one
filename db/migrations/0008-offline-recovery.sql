...
CREATE TABLE ppo.sync_acceptances (
 ...
 FOREIGN KEY(receipt_id) REFERENCES ppo.operation_receipts(id),
 FOREIGN KEY(workspace_id,appointment_id) REFERENCES ppo.appointments(workspace_id,id),
 CHECK(envelope->>'operation_id'=operation_id::text AND envelope->>'actor_id'=actor_id::text AND envelope->>'workspace_id'=workspace_id::text AND envelope->>'appointment_id'=appointment_id::text)
);
