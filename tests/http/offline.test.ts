...
test("P08 real HTTP bounded sync, original receipts, malformed siblings and actor/file isolation", async () => {
  ...
  const actor = (await call(p, "local-session")).body,
    context = await call(p, `sync/context/${setup.appointment_id}`, {});
  assert.equal(context.status, 200);
  assert.equal(context.cache, "private, no-store");
  assert.equal("finance" in context.body.job, false);
  assert.equal(context.body.owner.actor_id, actor.actor_id);
  let job = (await call(p, `my-jobs/${setup.appointment_id}`)).body.items[0];
  const start = operation(actor, job, "Start", startInput(job));
  const accepted = await call(p, "sync/operations", { operations: [start] });
  assert.equal(accepted.status, 200);
  assert.equal(accepted.body.outcomes[0].state, "ServerSaved");
  job = (await call(p, `my-jobs/${job.id}`)).body.items[0];
  const op = operation(actor, job, "Capture", entry(job)),
    bad = rehash({
      ...op,
      payload: {
        ...op.payload,
  ...
  const result = await call(p, "sync/operations", { operations: [bad, op] });
  assert.equal(result.body.outcomes[1].state, "ServerSaved");
  assert.notEqual(result.body.outcomes[0].state, "ServerSaved");
  assert.deepEqual(
    (await call(p, "sync/operations", { operations: [op] })).body.outcomes[0]
      .receipt,
    result.body.outcomes[1].receipt,
  );
  assert.equal(
    (await call(m, "sync/operations", { operations: [op] })).body.outcomes[0]
      .code,
    "RecordUnavailable",
  );
  const missing = await call(p, `sync/recovery/${randomUUID()}`);
  assert.equal(missing.status, 404);
  assert.equal(missing.body.code, "RecordUnavailable");
  assert.equal(
    (await call(p, "sync/operations", { operations: Array(21).fill(op) }))
      .status,
    422,
  );
  assert.equal(
  ...
  assert.equal(
    (await call(p, "sync/operations", { operations: [op] })).status,
    401,
  );
});
