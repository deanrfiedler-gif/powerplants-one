...
test("upgrade preserves saved CRM, mailbox, sessions, old grants and invitation limits; retry is idempotent", async () => {
  ...
  assert.equal(addedUsers.length,11);
  ...
  assert.equal((await database().query("SELECT 1 FROM ppo.permission_grants WHERE user_id=ANY($1::uuid[]) AND (capability LIKE 'engineering.material.%' OR capability LIKE 'engineering.change.%')", [users])).rowCount, 0);
