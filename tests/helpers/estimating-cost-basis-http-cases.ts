...
export function registerEstimatingCostBasisHttpCases() {
test("E2 costing HTTP binds exact revisions, preserves original responses and refuses wrong identity, source, origin and schema",async()=>{
  ...
  const forbidden=await fetch(origin+"/api/v1/"+path,{method:"POST",headers:{Cookie:cookie,Origin:"https://unrelated.invalid","Content-Type":"application/json"},body:JSON.stringify(input)});
