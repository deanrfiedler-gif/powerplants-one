...
const specs: Spec[] = [
  { ref: "001", title: "Valve assembly substitution", category: "SupplierProductChange", discipline: "Hydraulics", location: "Greenhouse 01", system: "Irrigation supply", author: "author", owner: "author", due: "2031-09-22", drawing: ["H-102", "B", "C", "Greenhouse 01 isolation schedule"], rationale: "The specified isolation valve assembly has a long fictional lead time; an alternate assembly with the same connections is proposed.", compare: ["Isolation valve assembly", "IV-050", "IV-055"], lines: ["020"], target: "InReview", cost: "none" },
  { ref: "002", title: "Pump duty amendment", category: "RequirementChange", discipline: "Hydraulics", location: "Irrigation Shed 01", system: "Irrigation supply", author: "engineer", owner: "engineer", due: "2031-09-22", drawing: ["H-101", "C", "D", "Pump station hydraulic arrangement"], rationale: "The customer added a nursery pad, so the duty point of the irrigation pump is re-examined.", compare: ["Duty flow", "SYN basis value r02", "SYN proposed value r03"], lines: ["010"], target: "Assessing", cost: "none" },
  { ref: "003", title: "Control interface revision", category: "SupplierProductChange", discipline: "Controls", location: "Irrigation Shed 01", system: "Controls", author: "engineer", owner: "engineer", due: "2031-09-23", drawing: ["E-201", "C", "D", "Irrigation control interface wiring"], rationale: "The CI-100 control interface is superseded by its fictional supplier; the CI-120 needs a revised termination arrangement on E-201.", compare: ["Control interface module", "CI-100", "CI-120 r2"], lines: ["030", "010"], asset: true, retest: { system: "Control interface", criterion: "Pump controller handshake and fail-safe state, as defined by the linked synthetic procedure", configuration: "CI-120 r2 terminated to E-201 revision D", procedure: "TP-E201" }, cost: "impact", target: "Accepted", purpose: "Procurement" },
  { ref: "004", title: "Sensor relocation", category: "SiteDiscrepancy", discipline: "Controls", location: "Propagation House", system: "Climate sensing", author: "author", owner: "author", due: "2031-09-24", drawing: ["E-204", "A", "B", "Propagation house sensor layout"], rationale: "The as-found bench layout shades the specified sensor position; a relocated mounting point is proposed.", compare: ["Sensor mounting position", "Bench row 2", "Bench row 4"], target: "InReview", cost: "none" },
  { ref: "005", title: "Pipework reroute", category: "InterfaceConflict", discipline: "Hydraulics", location: "Greenhouse 02", system: "Irrigation distribution", author: "author", owner: "engineer", due: "2031-09-25", drawing: ["H-110", "B", "C", "Greenhouse 02 distribution pipework"], rationale: "The specified pipe route clashes with a fictional structural brace; a reroute is proposed.", compare: ["Pipe route", "Along gridline C", "Along gridline D"], target: "Returned", cost: "none" },
  { ref: "006", title: "Filter access clearance", category: "DesignCorrection", discipline: "Mechanical", location: "Irrigation Shed 01", system: "Filtration", author: "author", owner: "author", due: "2031-09-26", drawing: ["M-106", "A", "B", "Filter station arrangement"], rationale: "The filter station leaves too little room to withdraw the disc stack; the arrangement is corrected.", compare: ["Withdrawal clearance", "SYN as drawn", "SYN as corrected"], evidenceNeeded: "Supplier withdrawal-clearance statement is outstanding.", target: "Assessing", cost: "none" },
  { ref: "007", title: "Commissioning logic update", category: "DesignCorrection", discipline: "Controls", location: "Shared controls", system: "Irrigation sequencing", author: "engineer", owner: "engineer", due: "2031-09-26", drawing: ["E-205", "B", "C", "Irrigation sequencing logic"], rationale: "The start sequence allows two zones to open together; the interlock logic is corrected.", compare: ["Zone interlock", "Permissive", "Exclusive"], retest: { system: "Commissioning logic", criterion: "Zone interlock sequence, as defined by the linked synthetic procedure", configuration: "Sequencing logic to E-205 revision C", procedure: "TP-E205" }, cost: "none", release: true, target: "Accepted", purpose: "Installation" },
...
export async function seedChangesScenario(as: SignIn, ids: ChangeScenarioIds, tag = "") {
  ...
  await confirm(people.engineer, "002", "impact-review", [request("002", "projects", "ImpactReview", "Projects", CHANGES.coordinator.id, "SYN say whether the added nursery pad changes the programme or the quoted scope.", "2031-09-25")]);
  ...
  await confirm(people.engineer, "003", "supply-review", [request("003", "supply", "ImpactReview", "SupplyChain", CHANGES.supply.id, "SYN say whether the CI-100 module is already ordered or received, and what its disposition would be.", "2031-09-24")]);
  ...
  await confirm(people.engineer, "007", "release-request", [request("007", "release", "PrepareRevisedRelease", "TechnicalRelease", CHANGES.releaseOwner.id, "SYN prepare and issue E-205 revision C for the corrected interlock logic.", "2031-09-23")]);
  ...
  await confirm(people.engineer, "007", "implementation", [
    request("007", "service", "Implementation", "Service", CHANGES.service.id, "SYN load the corrected sequencing logic at the next attendance.", "2031-09-25"),
    request("007", "commissioning", "Implementation", "Commissioning", CHANGES.verifier.id, "SYN retest the zone interlock against TP-E205 after the logic is loaded.", "2031-09-26")]);
