...
test("P11 PT-01/29 scoped recovery UI preserves originals and retries one uncertain disposition", async ({ page }, info) => {
  ...
  await page.getByRole("navigation", { name: info.project.use.isMobile ? "All modules" : "More navigation", exact: true }).getByRole("link", { name: "Integrations & recovery", exact: true }).press("Enter");
  await expect(page).toHaveURL(/\/admin\?department=sales$/);
