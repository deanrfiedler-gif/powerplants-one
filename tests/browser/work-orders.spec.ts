...
test("P04 forms retain invalid and stale proposals with explicit comparison; keyboard and phone layouts", async ({
  ...
  await expect(page.getByLabel("Task description", { exact: true })).toHaveAttribute("aria-invalid", "true");
...
test("P04 saved scope, reviewed readiness, authorisation, proposed visit and successor preserve approved evidence", async ({ page }, info) => {
  test.setTimeout(90000);
  await page.goto("/service/work-orders");
  await identity(page);
  async function post(path: string, fields: object) {
    const response = await page.request.post(`/api/v1/service/work-orders${path}`, {
      headers: { Origin: "http://127.0.0.1:3000" },
      data: { operation_id: crypto.randomUUID(), schema_version: 1, reason: "SYN browser fixture preparation", ...fields },
    });
    expect(response.ok(), await response.text()).toBe(true);
    return response.json();
  }
  const evidence = { title: "SYN reviewed authority", source_reference: "SYN-PPO-EVIDENCE-BROWSER", source_version: "1", content_text: "SYN manual review of external inspection only. No financial approval." };
  const wo = crypto.randomUUID();
  await post("", { id: wo, company_id: id(20), site_id: id(70), customer_id: id(50), service_owner_id: id(30), tickets: [{ ticket_id: id(40,20), issue_disposition: "SYN bounded browser review" }] });
  await post(`/${wo}/save-scope`, { expected_version: 1, scope: {
    summary: "SYN browser inspection", exclusions: "No intervention or extra work", diagnostic_limit: "External visual inspection only", pending_account_plan: "SYN Finance review remains outstanding", authority_evidence: evidence,
    coverage: { status: "Disputed", assessment: "SYN coverage dispute retained", reason: "SYN review pending", charging_route: "FinanceReview" },
    items: [{ task_kind: "Inspection", task_description: "Inspect external display", expected_outcome: "Record display state", completion_requirements: ["Record observations"], required_skill_codes: [], assets: [{ asset_id: id(80) }] }],
  } });
  await page.goto(`/service/work-orders/${wo}`);
  await page.getByText("Edit scope draft", { exact: true }).click();
  await page.getByLabel("Scope summary", { exact: true }).fill("SYN browser saved inspection scope");
  await page.getByRole("button", { name: "Save scope draft", exact: true }).click();
  await expect(page.getByRole("heading", { name: "SYN browser saved inspection scope", exact: true })).toBeVisible();
  async function savedOrder() { return (await (await page.request.get(`/api/v1/service/work-orders/${wo}`)).json()).items[0]; }
  for (const criterion_code of ["SiteControls", "CompetencyPlan", "MandatoryIsolation", "ShutdownAuthority"]) {
    const w = await savedOrder(), r = w.scopes[0];
    await post(`/${wo}/readiness`, { expected_version: w.version, assessment: { scope_revision_id: r.id, scope_version: r.version, criterion_code,
      outcome: ["MandatoryIsolation", "ShutdownAuthority"].includes(criterion_code) ? "NotApplicable" : "Pass",
      reason: "SYN reviewed non-intervention evidence", evidence, source_as_at: "2026-09-05T00:00:00Z" } });
  }
  await page.reload();
  await page.getByText("Review a readiness criterion", { exact: true }).click();
  await page.getByLabel("Readiness criterion", { exact: true }).selectOption("SiteAccess");
  await page.getByLabel("Readiness decision", { exact: true }).selectOption("Pass");
  await page.getByLabel("Review reason", { exact: true }).fill("SYN reviewed site access evidence");
  await page.getByLabel("Evidence source time (your device timezone)", { exact: true }).fill("2026-09-05T00:00");
  // Scope edit evidence fields are in a closed details; use the visible review form.
  const review = page.locator("form").filter({ has: page.getByRole("button", { name: "Record readiness review", exact: true }) });
  await review.getByLabel("Evidence title", { exact: true }).fill(evidence.title);
  await review.getByLabel("Synthetic source reference", { exact: true }).fill(evidence.source_reference);
  await review.getByLabel("Source version", { exact: true }).fill(evidence.source_version);
  await review.getByLabel("Exact manual evidence", { exact: true }).fill(evidence.content_text);
  await review.getByRole("button", { name: "Record readiness review", exact: true }).click();
  await expect(page.getByText("The saved scope is ready for an authorised reviewer’s decision.", { exact: true })).toBeVisible();
  await page.getByRole("button", { name: "Authorise current scope", exact: true }).click();
  await expect(page.getByText("Authorised scope — read-only", { exact: true })).toBeVisible();
  const approved = (await savedOrder()).scopes[0];
  await capture(page, info, "P04-authorised-through-ui.png");
  await page.reload();
  await page.getByText("Propose a visit", { exact: true }).click();
  await page.getByLabel("Proposed start (device timezone)", { exact: true }).fill("2026-09-20T09:00");
  await page.getByLabel("Proposed finish (device timezone)", { exact: true }).fill("2026-09-20T11:00");
  await page.getByRole("button", { name: "Save proposed visit", exact: true }).click();
  await expect(page.getByText("No visits proposed yet.", { exact: true })).toHaveCount(0);
  ...
  await page.getByLabel("Reason for successor scope", { exact: true }).fill("SYN additional external observations only; no shutdown or intervention");
  await page.getByLabel("Scope summary", { exact: true }).fill("SYN reviewed successor proposal");
  await page.getByRole("button", { name: "Create successor draft", exact: true }).click();
  await expect(page.getByText("Successor draft awaiting authorisation", { exact: true })).toBeVisible();
  const after = await savedOrder();
  expect(after.scopes.find((s: { id: string }) => s.id === approved.id).content_hash).toBe(approved.content_hash);
