...
test("P08 compatible IndexedDB upgrade preserves pending originals and exact Blob; abort and blocked upgrade retain evidence", async ({
  ...
  const result = await page.evaluate(async () => {
    ...
    const p = {
      ...
      display_name: "SYN storage-boundary fixture",
    };
    const id = crypto.randomUUID(),
      key = `${h.ownerKey(p)}:${id}`,
      bytes = new Blob([new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10])], {
        type: "image/png",
      });
    const original = {
      schema_version: 1,
      ...
      command: "AttachmentUpload",
      target_id: crypto.randomUUID(),
      authority: {
        assignment_id: crypto.randomUUID(),
        assignment_version: 1,
        schedule_version: 1,
        scope_revision_id: crypto.randomUUID(),
        scope_version: 1,
      ...
      payload: {
        operation_id: id,
        schema_version: 1,
        sha256: await h.sha256(new Uint8Array(await bytes.arrayBuffer())),
        byte_count: bytes.size,
      },
    };
    const wire = {
      ...original,
      payload_hash: await h.sha256(h.canonical(original)),
    };
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      tx.objectStore("bytes").put({
        key,
        owner: h.ownerKey(p),
        operation_id: id,
        bytes,
        sha256: wire.payload.sha256,
        byte_count: bytes.size,
      });
      tx.objectStore("status").put({
        key,
        owner: h.ownerKey(p),
        state: "LocalSaved",
        attempts: 0,
    ...
    old.close();
    const upgraded = await s.openStore();
    const version = upgraded.version,
      hasLease = upgraded.objectStoreNames.contains("leases");
    upgraded.close();
    const rows = await s.queue(p),
      file = await s.bytesFor(p, id);
    let abort = "";
    try {
      await s.localTransaction(
        ["operations"],
        "readwrite",
        (tx: IDBTransaction) => {
          tx.objectStore("operations").put({ key: "SYN-must-rollback" });
          tx.abort();
        },
      );
    } catch (e) {
    ...
    return {
      version,
      hasLease,
      unchanged: JSON.stringify(rows[0].original) === JSON.stringify(wire),
      blob: [...new Uint8Array(await file.bytes.arrayBuffer())],
      abort,
      after: after.length,
      blocked,
    };
  ...
  expect(result).toMatchObject({
    ...
    hasLease: true,
    unchanged: true,
    blob: [137, 80, 78, 71, 13, 10, 26, 10],
    after: 1,
  });
  expect(result.abort).toContain("AbortError");
  expect(result.blocked).toContain("blocked by another tab");
});

test("P08 unavailable IndexedDB and detected eviction refuse persistence claims", async ({
  page,
}) => {
...
test("P08 retained ownership locks deny cross-actor context, originals, bytes and receipts", async ({
  ...
  const result = await page.evaluate(async () => {
    ...
    const p = {
        actor_id: crypto.randomUUID(),
        workspace_id: crypto.randomUUID(),
        display_name: "SYN original owner",
      },
      other = {
        ...p,
        actor_id: crypto.randomUUID(),
        display_name: "SYN other owner",
      };
    await s.unlock(p);
    await s.lockLocal();
    const failures = [];
    for (const action of [
      () => s.contexts(p),
      () => s.queue(p),
      () => s.bytesFor(p, crypto.randomUUID()),
    ])
      try {
        await action();
      } catch (e) {
    ...
    return {
      failures,
      oldDenied,
      otherRows: await s.queue(other),
      otherContexts: await s.contexts(other),
    };
  });
  expect(result.failures).toHaveLength(3);
  expect(result.failures.every((x) => x.includes("locked or expired"))).toBe(
    true,
  );
  expect(result.oldDenied).toBe(true);
  expect(result.otherRows).toEqual([]);
  expect(result.otherContexts).toEqual([]);
});
...
test("P08 partial browser storage clearing is detected without a false receipt or recovery claim", async ({
  ...
  await page.evaluate(async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      const r = indexedDB.deleteDatabase(s.DB_NAME);
      r.onsuccess = () => resolve();
      r.onerror = () => reject(r.error);
    });
  });
