...
async function captureSizes(page: Page, outputPath: (name: string) => string, scope: string) {
  for (const width of [1440, 1024, 390, 320]) {
    await page.setViewportSize({width, height: width === 1440 ? 960 : width === 1024 ? 768 : 844});
    ...
    if (await header.isVisible()) {
      ...
      for (const control of await header.locator('button:visible').all()) {
        ...
        expect(rect.y + rect.height, 'Header controls must not overlay page actions').toBeLessThanOrEqual(bounds.y + bounds.height + 1);
      }
    }
    await page.screenshot({path:outputPath(`sales-${scope}-${width}.png`),fullPage:false});
  }
}
...
test("CR03 Won receiving returns, corrects and accepts an exact successor while Due remains immutable", async ({page}, info) => {
  const o = await deal(page, `SYN Won receiving ${randomUUID()}`);
  let version = 1;
  for (const stage_id of ["Scoping", "Quoting", "Negotiation", "Closing"])
    await call(page, `crm/opportunities/${o.id}/stage`, {...crmBase(), expected_version:version++, stage_id, qualification_note:null, identification_activity_id:null});
  await call(page, `crm/opportunities/${o.id}/outcome`, {...crmBase(), expected_version:version, close_outcome:"Won", lost_reason:null, acceptance_evidence:"SYN recorded acceptance; no conversion authority"});
  const original = (await call(page, `crm/opportunities/${o.id}`)).items[0];
  const id = randomUUID();
  await call(page, "sales/handovers", {...crmBase(), id, opportunity_id:o.id, kind:"Won"});
  await call(page, `sales/handovers/${id}`, {...crmBase(), expected_version:1, action:"Save", receiving_owner_id:o.owner_id, note:"SYN exact receiving preparation", content:{...emptyHandover(), problem:"SYN controls", outcome:"Usable controls", included_scope:"SYN installation", exclusions:"None", assumptions:"None", unknowns:"None", date_reason:"Customer date to be agreed", next_activity_id:o.initial_action.id, destination:"Projects", routing_basis:"SYN recorded owner direction", delivery_items:"SYN control panel", release_prerequisites:"Engineering release remains open; receiver owns follow-up"}});
  await page.goto(`/sales/handoffs/won/${id}`);
  await page.getByRole("tab", {name:"Review & clarification",exact:true}).click();
  await page.getByRole("button", {name:"Submit exact revision",exact:true}).click();
  await page.getByLabel("Review, clarification or return reason").fill("SYN clarify the selected installation boundary");
  await page.getByRole("button", {name:"Return with owned follow-up",exact:true}).click();
  await page.getByRole("button", {name:"Create successor revision",exact:true}).click();
  await page.getByRole("tab", {name:"Brief & scope",exact:true}).click();
  await page.getByLabel("Included scope", {exact:true}).fill("SYN corrected control panel installation boundary");
  await page.getByRole("button", {name:"Save draft",exact:true}).click();
  await page.getByRole("tab", {name:"Review & clarification",exact:true}).click();
  await page.getByRole("button", {name:"Submit exact revision",exact:true}).click();
  await page.getByLabel("Review, clarification or return reason").fill("SYN exact corrected revision received; release still open");
  await page.getByRole("button", {name:"Accept exact revision",exact:true}).click();
  await expect(page.getByRole("button", {name:"Create successor revision",exact:true})).toBeVisible();
  await page.reload();
  ...
  expect(accepted.record.content.release_prerequisites).toContain("Engineering release remains open");
  expect(accepted.history.map((e:{action:string})=>e.action)).toContain("Return");
...
test("CR05 issued source review saves attributed feedback, commercial preparation and guarded closure with responsive controls", async ({
  ...
  if (saved.record.state !== "Open") {
    await call(page, `sales/aftercare/${id}`, {...crmBase(), expected_version:saved.record.version, action:"CorrectReview", data:{}});
    saved = await call(page, `sales/aftercare/${id}`);
  }
  await call(page, `sales/aftercare/${id}`, {...crmBase(), expected_version:saved.record.version, action:"SaveReview", data:{review:emptyReview()}});
  ...
  await page.getByLabel("Date choice / commitment / reason date needed", {exact:true}).fill("SYN explicit customer choice; no automatic interval");
  await page.getByLabel("Customer review date", {exact:true}).fill("2026-09-24");
  await page.getByLabel("Review method and evidence limits", {exact:true}).fill("SYN customer telephone statements; no competence assessment");
  await page.getByRole("button", {name:"Add participant",exact:true}).click();
  await page.getByLabel("Participant", {exact:true}).selectOption(options.people[0].id);
  await page.getByLabel("Recorded role (does not imply signing authority)", {exact:true}).fill("SYN participant; signing authority not established");
  await page.getByRole("button", {name:"Add feedback",exact:true}).click();
  await page.getByLabel("Speaker / attributed person", {exact:true}).selectOption(options.people[0].id);
  await page.getByLabel("Statement and context", {exact:true}).fill("SYN customer reports the controls are usable");
  await page.getByLabel("Agreed next steps (or explicitly None)", {exact:true}).fill("None");
  await page.getByRole("button", {name:"Save review",exact:true}).click();
  await page.getByRole("button", {name:"Refresh review preparation",exact:true}).click();
  await page.getByRole("button", {name:"Complete customer review",exact:true}).click();
  await expect(page.getByRole("button", {name:"Open correction",exact:true})).toBeVisible();
  await page.getByRole("tab", {name:"Maintenance & renewal",exact:true}).click();
  await page.getByLabel("Source observation and evidence", {exact:true}).fill("SYN customer review recorded");
  await page.getByLabel("Identified customer need", {exact:true}).fill("SYN explore a later controls review");
  await page.getByLabel("Unresolved assumptions", {exact:true}).fill("No renewal authority or agreement terms adopted");
  await page.getByLabel("I reviewed the permitted existing-opportunity source", {exact:true}).check();
  await page.getByRole("button", {name:"Prepare commercial discussion",exact:true}).click();
  await page.getByRole("tab", {name:"Customer review",exact:true}).click();
  await page.getByRole("button", {name:"Close aftercare record",exact:true}).click();
