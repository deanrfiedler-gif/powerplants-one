...
test("ES01 native workload supports keyboard filters, exact discovery entry, reload, history and responsive cards", async ({
  ...
  await page.getByText("Readiness, owner and sort", { exact: true }).click();
  await expect(
    page.getByRole("combobox", { name: "Scope readiness", exact: true }),
  ).toBeVisible();
  await page
  ...
  const choicesLoaded = page.waitForResponse(
    (response) => new URL(response.url()).pathname === "/api/v1/estimating/options",
  );
  await page
    .getByRole("link", { name: "Start discovery", exact: true })
    .click();
