...
/* Contract fidelity */
test('Capability catalogue equals the application Capability union exactly',()=>{same([...C.keys],union);assert.equal(C.keys.length,65);assert.equal(C.source_sha256,sha(path.join(root,'src/platform/permissions.ts')));});
...
test('Every capability has a plain-English label and a known family',()=>{for(const c of M.catalogue()){assert(c.label&&c.label.length>5,c.key);assert(c.familyName,c.key);}assert.equal(Object.keys(M.LABELS).length,65);});
