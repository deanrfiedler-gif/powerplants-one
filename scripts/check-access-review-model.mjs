...
test('Hosted demo capability set equals scripts/demo-database.ts',()=>{same([...C.hosted_demo],demo);assert.equal(C.hosted_demo.length,24);assert.equal(C.demo_source_sha256,sha(path.join(root,'scripts/demo-database.ts')));});
...
test('Hosted testers mirror setup: Company A, tester set, end at tester expiry',()=>{for(const h of [H1,H2]){const t=state.testers.find(x=>x.userId===h),gs=state.grants.filter(g=>g.userId===h);assert.equal(gs.length,24);assert(gs.every(g=>g.scopeType==='Company'&&g.companyId===CA&&g.validTo===t.expiresAt));}assert.match(demoText,/Exactly the bounded Company A grant set/);});
