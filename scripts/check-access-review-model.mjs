...
/* Contract fidelity */
test('Capability catalogue equals the application Capability union exactly',()=>{same([...C.keys],union);assert.equal(C.keys.length,61);assert.equal(C.source_sha256,sha(path.join(root,'src/platform/permissions.ts')));});
test('Hosted demo capability set equals scripts/demo-database.ts',()=>{same([...C.hosted_demo],demo);assert.equal(C.hosted_demo.length,27);assert.equal(C.demo_source_sha256,sha(path.join(root,'scripts/demo-database.ts')));});
test('Every capability has a plain-English label and a known family',()=>{for(const c of M.catalogue()){assert(c.label&&c.label.length>5,c.key);assert(c.familyName,c.key);}assert.equal(Object.keys(M.LABELS).length,61);});
...
test('Hosted testers mirror setup: Company A, tester set, end at tester expiry',()=>{for(const h of [H1,H2]){const t=state.testers.find(x=>x.userId===h),gs=state.grants.filter(g=>g.userId===h);assert.equal(gs.length,27);assert(gs.every(g=>g.scopeType==='Company'&&g.companyId===CA&&g.validTo===t.expiresAt));}assert.match(demoText,/Exactly the bounded Company A grant set/);});
