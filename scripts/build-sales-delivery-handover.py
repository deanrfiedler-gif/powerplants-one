...
Line endings. The repository's .gitattributes pins .md, .py, .json, .yml, .csv
and .html to LF, but .css and .js fall under `* text=auto`, so Git checks those
out with CRLF on Windows. The pinned hashes below therefore compare content with
newlines normalised rather than raw bytes, and the output is written as bytes so
no platform translates them on the way out. Same input, same hash, same output,
on every platform.
"""
...
# document. Its content must not have moved under the design.
...
actual = content_sha256(quotation)
...
# source rather than duplicated. Pin the content so a change upstream is a reviewed
...
fonts_actual = content_sha256(fonts)
...
# Bytes, not write_text: no platform-dependent newline translation on the way out.
