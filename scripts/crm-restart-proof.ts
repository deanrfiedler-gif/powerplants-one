...
try {
  ...
  if (phase === "write") {
    const input = {
        ...crmCreate(),
        title: "SYN I1 durable opportunity after process restart",
      },
      created = await call("crm/opportunities", input);
    const completed = {
      ...crmBase(),
      expected_version: 1,
      outcome:
        "SYN Exact first outcome retained across application and PostgreSQL restarts",
    };
    const completionReceipt = await call(
      `activities/${input.initial_action.id}/complete`,
      completed,
    );
    const qualification = crmQualify();
    const qualificationReceipt = await call(
      `crm/opportunities/${input.id}/qualify`,
      qualification,
    );
    const action = {
      ...crmBase(),
    ...
    const planned = await call(
      `crm/opportunities/${input.id}/next-action`,
      action,
    );
    const operations = [
      created,
    ...
    const db = (
      await database().query(
        "SELECT r.operation_id,r.payload_hash,r.result FROM ppo.operation_receipts r WHERE r.operation_id=ANY($1::uuid[]) ORDER BY operation_id",
    ...
    assert.equal(o.display_number, proof.detail.display_number);
    assert.equal(o.next_action_state, "DueNeeded");
    assert.equal(o.actions.length, 2);
    assert.equal(o.actions[0].outcome, proof.detail.actions[0].outcome);
    assert.deepEqual(o.events, proof.detail.events);
    for (const receipt of proof.operations)
      assert.deepEqual(
        await call(`operations/${receipt.operation_id}`),
        receipt,
      );
    const db = (
      await database().query(
        "SELECT r.operation_id,r.payload_hash,r.result FROM ppo.operation_receipts r WHERE r.operation_id=ANY($1::uuid[]) ORDER BY operation_id",
        [proof.operations.map((r: { operation_id: string }) => r.operation_id)],
      )
    ).rows;
    assert.deepEqual(db, proof.db);
    const replay = await call("crm/opportunities", proof.input);
    assert.deepEqual(replay, proof.operations[0]);
    assert.equal(
      (
        await database().query(
          "SELECT count(*)::int AS n FROM ppo.opportunities WHERE id=$1",
          [proof.input.id],
        )
      ).rows[0].n,
      1,
    ...
    assert.equal(
      (
        await database().query(
          "SELECT count(*)::int AS n FROM ppo.activity_links WHERE opportunity_id=$1",
          [proof.input.id],
        )
      ).rows[0].n,
      2,
  ...
  await page.goto(origin + `/crm/opportunities/${proof.input.id}`);
  await expect(
    ...
    JSON.stringify(
      {
        ...
        opportunity_id: proof.input.id,
        operations: proof.operations.map(
          (r: { operation_id: string }) => r.operation_id,
        ),
        database_version: (
          await database().query("SELECT version() AS version")
        ).rows[0].version,
      },
      null,
      2,
