...
try {
  ...
  if (process.argv[2] === "write") {
    ...
    const successor = {
      ...
      reason: "SYN corrected actual quantity before database restart",
    };
    ...
    await captureEntry(
      ...
      entry(job, "Photo", {
        ...
        caption: "SYN persisted original inspection fixture",
      }),
    );
    job = (await readFieldJob(tech, job.id)).items[0];
    const completion = draft(job);
    await saveCompletionDraft(tech, job.id, completion);
    await writeFile(
      "/tmp/ppo-p07-restart.json",
      JSON.stringify({
        appointment_id: job.id,
        authority_hash: job.attendance.authority_hash,
        original_entry_id: original.id,
        corrected_entry_id: successor.id,
        attachment_id: image.id,
        photo_hash: digest(image.bytes),
        draft_id: completion.id,
    ...
    const proof = JSON.parse(
        await readFile("/tmp/ppo-p07-restart.json", "utf8"),
      ),
      tech = (await createSession("assigned-technician")).principal;
    ...
    assert.equal(
      job.entries.find((e) => e.id === proof.original_entry_id)?.payload
        .quantity,
      "2",
    );
    assert.equal(
      job.entries.find((e) => e.id === proof.corrected_entry_id)
        ?.supersedes_entry_id,
      proof.original_entry_id,
    );
    assert.equal(
      digest((await attachmentBytes(tech, proof.attachment_id)).bytes),
      proof.photo_hash,
    );
    assert.deepEqual(
      await readOperation(tech, proof.operation_id),
      proof.receipt,
    );
    console.log(
      "P07 PostgreSQL process restart: exact start authority, original/corrected capture lineage, durable verified PNG, completion draft, owned follow-up and original receipt verified",
    );
