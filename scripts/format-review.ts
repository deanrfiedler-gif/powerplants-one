...
for (const path of paths) {
  if (!/\.(ts|tsx|js|css|html)$/.test(path)) continue;
  const source = await readFile(path, "utf8"),
    formatted = await prettier.format(source, { filepath: path });
  const out = `verification-evidence/review-format/${path}.txt`;
