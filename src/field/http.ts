...
export async function uploadRoute(request: NextRequest, context: RouteContext) {
  try {
    ...
    const result = await uploadAttachment(
      p,
      id ?? "",
      await jsonBody(request, 5_600_000),
    );
...
export async function photoRoute(request: NextRequest, context: RouteContext) {
  try {
    localRequest(request);
    object(Object.fromEntries(request.nextUrl.searchParams), []);
    const p = await identity(request),
      { id } = await context.params,
      { bytes, filename } = await attachmentBytes(p, id ?? "");
