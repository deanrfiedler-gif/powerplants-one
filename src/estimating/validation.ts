...
export function parseLines(value: unknown, schemaVersion: 1 | 2 = 1): CostLine[] {
  if (!Array.isArray(value) || value.length > 100) invalid("lines", "Use at most 100 complete manual lines.");
  const lines = value.map((item, i) => {
    const r = object(item, ["id", "description", "category", "quantity", "unit", "unit_cost", "unit_sell", "source", "effective_date", ...(schemaVersion === 2 ? ["allowance"] : [])]), f = (name: string) => `lines.${i}.${name}`;
    if (schemaVersion === 2 && typeof r.allowance !== "boolean") invalid(f("allowance"), "Record an explicit Yes or No allowance choice.");
    ...
    return { id: uuid(r.id, f("id")), description: label(r.description, f("description"), 300),
      category: choice(r.category, f("category"), schemaVersion === 2 ? ["Product", "Labour", "Freight", "Engineering", "Subcontract"] as const : ["Product", "Labour", "Freight"] as const),
      ...(schemaVersion === 2 ? { allowance: r.allowance as boolean } : {}),
...
function estimateEnvelope(r: Record<string, unknown>) {
  ...
  return { operation_id: uuid(r.operation_id, "operation_id"), schema_version: 2 as const, reason: label(r.reason, "reason", 1000) };
}
export function createInput(value: unknown) {
  ...
  const envelope = estimateEnvelope(r);
  return { ...envelope, id: uuid(r.id,"id"), opportunity_id: uuid(r.opportunity_id,"opportunity_id"), owner_id: uuid(r.owner_id,"owner_id"),
    title: label(r.title,"title",200), scope: scope(r.scope), lines: parseLines(r.lines, envelope.schema_version), policy: policy(r.policy) };
}
export function saveInput(id: string, value: unknown) {
  ...
  const envelope = estimateEnvelope(r);
  return { ...envelope, id: uuid(id,"id"), expected_version: version(r.expected_version), title: label(r.title,"title",200), scope: scope(r.scope), lines: parseLines(r.lines, envelope.schema_version), policy: policy(r.policy) };
}
