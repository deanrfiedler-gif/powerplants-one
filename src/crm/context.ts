...
export async function crmAvailable(c: QueryClient) {
  return !!(
    await c.query("SELECT to_regclass('ppo.opportunities') AS relation")
  ).rows[0].relation;
}
export function opportunityVisibility(alias = "o") {
  return `${scopeSql(`${alias}.company_id`, `${alias}.site_id`, "crm.opportunity.read")}
    ...
    AND EXISTS(SELECT 1 FROM ppo.organisations crm_org WHERE crm_org.workspace_id=${alias}.workspace_id AND crm_org.id=${alias}.organisation_id AND ${visibility("Organisation", "crm_org")})
...
export async function visibleOpportunity(
  c: QueryClient,
  p: Principal,
  id: string,
) {
...
export async function eligibleOpportunityOwner(
  c: QueryClient,
  p: Principal,
  input: OpportunityContext,
...
export async function eligibleActionOwner(
  c: QueryClient,
  p: Principal,
  input: OpportunityContext,
  ownerId: string,
...
export async function opportunityAuthority(
  c: QueryClient,
  p: Principal,
  id: string,
  cap: "crm.opportunity.create" | "crm.opportunity.edit",
  owned = true,
) {
  ...
  await c.query(
    "SELECT id FROM ppo.opportunities WHERE workspace_id=$1 AND id=$2 FOR UPDATE",
    [p.workspace_id, id],
  );
  await relationshipContext(c, p, o, cap);
  await eligibleOpportunityOwner(c, p, o);
  if (owned && o.owner_id !== p.actor_id)
    throw new AppError(
      403,
      "CRM_OWNER_REQUIRED",
      "Only the current opportunity owner can record qualification or plan its next action.",
    );
  return o;
}
