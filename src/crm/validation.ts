...
export function parseCreate(value: unknown) {
  ...
  if ((site_id === null) !== (site_unknown_reason !== null))
    invalid(
      "site_unknown_reason",
      "Explain an unknown site, or select a site and clear its unknown reason.",
    );
  ...
  return {
    ...
    source_channel: choice(r.source_channel, "source_channel", [
      "Phone",
      "Email",
      "Meeting",
      "Referral",
...
export function parsePlan(id: string, value: unknown) {
  ...
  if ((activity_id !== null) === (r.new_action != null))
    invalid(
      "activity_id",
      "Choose an existing active action or provide one new action.",
    );
  return {
    ...common(r),
    id: uuid(id, "id"),
