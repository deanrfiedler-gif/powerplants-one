...
export function packHtml(
 ...
 </style></head><body><main><header><p class="classification">Synthetic prototype — not for operational use</p><p>POWERPLANTS ONE · SERVICE OPERATIONS</p><h1>Technician job pack</h1><p><strong>${e(s.pack_reference)} · ${revision}</strong></p><p>${e(s.customer.name)} · ${e(s.site.name)}</p><p class="meta">Work ${e(s.work.reference)} · Appointment ${e(s.appointment.reference)} · ${e(s.appointment.timezone)}</p><p class="meta">${output ? `Issue timestamp ${e(output.prepared_at)} (reserved at output preparation; effective only on application release) · Issue reference ${e(output.issue_id)}. The application records actual release time and current status. This file alone is not proof of release.` : "Preparation preview — not issued"}</p></header>
