...
export function validatePayload(op: WireOperation) {
  ...
  if(op.command === "SubmitCompletion" && b.draft_revision_id && typeof b.draft_revision_id === "object") {
    const ref = object(b.draft_revision_id,["operation_id"]), id=uuid(ref.operation_id,"draft_operation_id");
    if(!op.depends_on.includes(id)) throw new AppError(422,"InvalidDependencies","Declare the exact original completion-draft dependency.");
...
async function resolvedPayload(p: Principal, op: WireOperation) {
  ...
  if(op.command==="SubmitCompletion" && b.draft_revision_id && typeof b.draft_revision_id==="object") {
    const dependency=(b.draft_revision_id as {operation_id:string}).operation_id;
    const d=(await database().query("SELECT id FROM ppo.completion_draft_revisions WHERE workspace_id=$1 AND actor_id=$2 AND appointment_id=$3 AND operation_id=$4",[p.workspace_id,p.actor_id,op.appointment_id,dependency])).rows[0];
    if(!d)throw new AppError(409,"DependencyPending","The exact original completion draft has not been accepted.");
...
async function validateAuthority(
  ...
  const attendanceId =
    b.attendance_id ??
    (op.command==="CustomerResponse" ? (await c.query("SELECT attendance_id FROM ppo.service_reports WHERE workspace_id=$1 AND actor_id=$2 AND appointment_id=$3 AND id=$4",[p.workspace_id,p.actor_id,op.appointment_id,op.target_id])).rows[0]?.attendance_id : undefined) ??
    (
      await c.query(
