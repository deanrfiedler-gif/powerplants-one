import { NavigationIcon, isNavigationIcon, type NavigationIconName } from "./navigation-icons";
const paths = {
  eye: "M2 12s4-7 10-7 10 7 10 7-4 7-10 7S2 12 2 12 M15 12a3 3 0 1 0-6 0 3 3 0 0 0 6 0",
  archive: "M3 3h18v5H3z M5 8v13h14V8 M9 12h6",
  chevron: "m8 10 4 4 4-4",
  collapse: "m14 6-6 6 6 6",
  expand: "m10 6 6 6-6 6",
  changes: "M3 7h15l-4-4 M18 7l-4 4 M21 17H6l4-4 M6 17l4 4",
  download: "M12 3v12m-5-5 5 5 5-5 M4 17v4h16v-4",
  flag: "M5 21V3h14v11H5 M5 3l14 11 M19 3 5 14",
  trash: "M3 6h18 M9 6V3h6v3 M5 6l1 15h12l1-15 M10 10v7 M14 10v7",
  check: "m5 12 4 4L19 6",
  info: "M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18 M12 11v6 M12 7h.01",
  search: "M10.5 3a7.5 7.5 0 1 0 0 15 7.5 7.5 0 0 0 0-15 M16 16l5 5",
  mail: "M3 5h18v14H3z M3 5l9 8 9-8",
  edit: "m15 4 5 5 M3 21l5-1L21 7l-5-5L3 15z",
  home: "m3 10 9-7 9 7v10H3z M9 20v-7h6v7",
  work: "M8 4H5v17h14V4h-3 M8 3h8v4H8z M8 12h8 M8 16h5",
  sales: "M3 7h18v14H3z M8 7V3h8v4 M3 12l9 4 9-4 M10 12h4",
  leads: "M3.5 5h17l-6.75 8v5.25l-3.5 1.75V13z",
  pulse: "M3 12h3.25l2.25-5.5 3.5 11 2.5-5.5H21",
  deals: "M12 3.5v17 M15.25 8.25A2.75 2.75 0 0 0 12.5 6h-1a2.5 2.5 0 0 0 0 5h1a2.5 2.5 0 0 1 0 5h-1a2.75 2.75 0 0 1-2.75-2.25",
  calendar: "M4 6h16v14H4z M8 3.5v4 M16 3.5v4 M4 10.5h16 M8 14h1.5 M11.25 14h1.5 M8 17.5h1.5 M11.25 17.5h1.5",
  products: "M3.5 8.25 12 4.5l8.5 3.75v7.5L12 19.5l-8.5-3.75z M3.5 8.25 12 12l8.5-3.75 M12 12v7.5 M7.75 6.4l8.5 3.75",
  insights: "M3.5 20.5h17 M6.75 20.5v-5.5 M11.5 20.5V9 M16.25 20.5v-8.5",
  estimate: "M5 2h14v20H5z M8 6h8 M8 11h1 M12 11h1 M16 11h1 M8 15h1 M12 15h1 M16 15h1 M8 19h1 M12 19h1 M16 19h1",
  engineering: "m3 17 14-14 4 4L7 21z M13 7l4 4 M10 10l2 2 M7 13l4 4",
  projects: "M3 5h7l2 3h9v13H3z",
  service: "M4 5h16v16H4z M8 2v6 M16 2v6 M4 11h16 M8 15h3 M8 18h7",
  supply: "m3 7 9-5 9 5v10l-9 5-9-5z M3 7l9 5 9-5 M12 12v10 M7 4l10 6",
  finance: "M5 3l3 2 4-2 4 2 3-2v19l-3-2-4 2-4-2-3 2z M8 9h8 M8 13h8 M8 17h5",
  customers: "M9 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8 M2 21v-3a7 7 0 0 1 14 0v3 M16 4a4 4 0 0 1 0 8 M18 15a5 5 0 0 1 4 5v1",
  person: "M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8 M4 21v-2a8 8 0 0 1 16 0v2",
  settings: "M4 7h16 M4 17h16 M8 4v6 M16 14v6",
  board: "M3 4h5v16H3z M10 4h5v11h-5z M17 4h4v14h-4z",
  list: "M8 5h13 M8 12h13 M8 19h13 M3 5h1 M3 12h1 M3 19h1",
  plus: "M12 4v16 M4 12h16",
  filter: "M3 5h18 M6 12h12 M9 19h6",
  clock: "M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18 M12 7v5l3 2",
  warning: "m12 3 10 18H2z M12 9v5 M12 17v1",
  menu: "M3 6h18 M3 12h18 M3 18h18",
  close: "m5 5 14 14 M5 19 19 5",
  more: "M5 11a1 1 0 1 0 0 2 1 1 0 0 0 0-2 M12 11a1 1 0 1 0 0 2 1 1 0 0 0 0-2 M19 11a1 1 0 1 0 0 2 1 1 0 0 0 0-2",
  help: "M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20 M9 8a3 3 0 1 1 5 2c-1 1-2 1-2 4 M12 17h.01",
  bell: "M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9 M9 21h6 M12 1v1",
  sites: "M12 22s8-8 8-13a8 8 0 0 0-16 0c0 5 8 13 8 13 M12 6a3 3 0 1 0 0 6 3 3 0 0 0 0-6",
  documents: "M5 2h9l5 5v15H5z M14 2v6h5 M8 12h8 M8 16h8",
  inbox: "M22 12h-6l-2 3h-4l-2-3H2 M5.45 5.11 2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z",
  caret: "M7 10h10l-5 6z",
  sort: "M3 5h10 M3 11h7 M3 17h4 M17 4v16 M12 15l5 5 5-5",
  disqualified: "M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20 M4.93 4.93l14.14 14.14",
  converted: "M22 11.08V12a10 10 0 1 1-5.93-9.14 M22 4 12 14.01l-3-3",
  gear: "M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6 M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 1 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 1 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 1 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z",
};
export type ProductIconName = keyof typeof paths | NavigationIconName;
export function ProductIcon({ name, active = false }: { name: ProductIconName; active?: boolean }) {
  if (isNavigationIcon(name)) return <NavigationIcon name={name} active={active} />;
  return <svg className="product-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d={paths[name]} /></svg>;
}
