...
type Drawing = { body: ReactNode; detail?: ReactNode; open?: boolean; selected?: ReactNode };
...
const bindings = <path d="M7 2v5M17 2v5M3 10h18" />;
...
export const navigationDrawings = {
  ...
  "nav-activities": { body: calendar, detail: <>{bindings}<path d="M7 14h1M12 14h1M17 14h.01M7 18h1M12 18h1M17 18h.01"/></> },
  "nav-tasks": { body: box, detail: <>{clip}<path d="m8 13 3 3 5-6"/></> },
  ...
  "nav-work": { body: box, detail: <>{clip}<path d="M8 10h8M8 14h8M8 18h5"/></> },
  ...
  "nav-schedule": { body: calendar, detail: <>{bindings}<path d="M7 15h10M7 13v4M17 13v4"/></> },
  ...
  "nav-inbound": { body: <rect x="2" y="5" width="20" height="14" rx="1"/>, detail: <path d="M6 8v8M10 8v8M14 8v8M18 8v8M4 19v2M20 19v2"/> },
  "nav-receiving": { body: <path d="m12 3 9 5v5l-7 9-11-5V8Z"/>, detail: <>{seams}<path d="m16 18 2 2 4-5"/></> },
  "nav-stock": { body: <path d="M2 9 12 3l10 6v13H2Z"/>, detail: <path d="M6 22V11h12v11M6 15h12M6 19h12"/> },
  "nav-dispatch": { body: <><path d="M2 5h12v13H2Zm12 5h4l4 5v3h-8Z"/></>, detail: <><circle cx="6" cy="19" r="2.5"/><circle cx="18" cy="19" r="2.5"/><path d="M15 10v5h7"/></> },
  "nav-returns": { body: <path d="m12 3 9 5v5l-7 9-11-5V8Z"/>, detail: <>{seams}<path d="m17 16 5 5m0-5-5 5"/></> },
  "nav-accounts": { body: <rect x="4" y="2" width="16" height="20" rx="2"/>, detail: <><path d="M7 2v20M2 6h4M2 12h4M2 18h4M10 17a4 4 0 0 1 7 0"/><circle cx="13.5" cy="9" r="2.5"/></> },
  ...
  "nav-facilities": { body: <path d="m2 15 13-7 7 7-13 7Z"/>, detail: <path d="M2 15v-5M15 8V3M22 15v-5M9 22v-5M8 12l7 7"/> },
  "nav-documents": { body: <path d="M8 2h9l4 4v13H8Z"/>, detail: <path d="M17 2v5h4M5 6H3v16h13M11 11h7M11 15h7"/> },
  "nav-screen": { body: <rect x="3" y="3" width="16" height="15" rx="1"/>, detail: <path d="M3 7h16M3 11h16M3 15h16M22 3v18M21 21h2"/> },
  ...
  "nav-sales": { body: <rect x="3" y="6" width="18" height="15" rx="2"/>, detail: <path d="M8 6V3h8v3M3 11l9 4 9-4M10 13v4h4v-4"/> },
...
export function NavigationIcon({ name, active = false }: { name: NavigationIconName; active?: boolean }) {
  ...
  return <svg className="product-icon" data-icon={name} data-variant={active ? "active" : "outline"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
    {active && drawing.selected ? drawing.selected : <>
      ...
      {drawing.detail && <g stroke={active && !drawing.open ? "var(--ppo-icon-cutout, #f0f6ed)" : "currentColor"}>{drawing.detail}</g>}
    </>}
  </svg>;
}
