  ...
  return <div className="assistant-screen">
    ...
    <div className="assistant-grid">
      ...
      <div className="assistant-workspace">
        ...
        {turn&&'opportunities' in turn&&!proposal&&<section className="assistant-card"><h2>Opportunity worklist</h2><p>Owner: {turn.mine?'my opportunities':'all permitted owners'} · Next action: {turn.next_action??'any'} · First 20 results.</p><ul className="assistant-records">{turn.opportunities.items.map(o=><li key={o.id}><Link href={`/crm/opportunities/${o.id}`}>{o.title} · {o.display_number}</Link></li>)}</ul>{!turn.opportunities.items.length&&<p>No permitted matches in this filtered result.</p>}<Link href={`/crm/opportunities?${new URLSearchParams({...turn.next_action?{next_action:turn.next_action}:{}})}`}>Continue in the full CRM worklist</Link></section>}
