...
export function denied(error:unknown) {return [401,403,404].includes((error as Failure)?.status ?? 0);}
...
export function useCrmResource<T>(path:string|null) {
 const [revision,setRevision]=useState(0),[state,setState]=useState<{path:string|null;data:T|null;error:unknown}>({path:null,data:null,error:null});
 useEffect(()=>{
  let live=true;
  const load=()=>{if(path)void api<T>(path).then(data=>{if(live)setState({path,data,error:null});},error=>{if(live)setState(old=>({path,data:!denied(error)&&old.path===path?old.data:null,error}));});};
 ...
 return {data:state.path===path?state.data:null,error:state.path===path?state.error:null,loading:!!path&&(state.path!==path||(!state.data&&!state.error)),reload:()=>setRevision(x=>x+1)};
}
export function useCrmCommand(onAccepted:(r:OperationReceipt)=>void) {
 const [busy,setBusy]=useState(false),[error,setError]=useState<unknown>(null),[status,setStatus]=useState("Unsaved"),[uncertain,setUncertain]=useState(false);
 const pending=useRef<{path:string;body:Record<string,unknown>}|null>(null);
 async function execute(intent:{path:string;body:Record<string,unknown>},lookup:boolean) {
  setBusy(true);setError(null);
  try {
   let receipt:OperationReceipt|null=null;
   if(lookup) try {receipt=await api<OperationReceipt>(`operations/${intent.body.operation_id}`);}catch(e){if((e as Failure).status!==404)throw e;}
   ...
   pending.current=null;setUncertain(false);setStatus("Saved to the server");onAccepted(receipt);
